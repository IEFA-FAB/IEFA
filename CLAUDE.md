...
- **Lint de Tailwind (`@shadcn/lint` pelo Oxlint)**: `bun run lint:tailwind`, também dentro do `bun run check` e do `lint` de cada app com `components.json` (então roda nos `check-<app>` do CI). Config em `.oxlintrc.tailwind.jsonc` — nome fora do padrão de propósito, o react-doctor adotaria um `.oxlintrc.json`.
  ...
  - **Valor fora da escala (`no-arbitrary-values`) é aviso; dimensão de layout passa.** Resolve-se pela classe semântica do STYLE_CONTRACT do app (`.text-hint`, `.text-label`, `var(--tracking-label)`…), **não trocando `text-[10px]` por um degrau cru ou token novo**: isso só renomeia a dívida e a tira da contagem.
  - **`no-restyle`: `className` em componente de `components/ui` só posiciona** (layout: margem, largura, flex). Cor, forma, padding e tipografia de `Button`/`Input`/`Badge`/`SelectTrigger` vêm de variante — faltando uma, ela nasce no primitivo. Os slots abrem o que existem para receber (contratos em `.oxlintrc.tailwind.jsonc`): espaçamento em `Card*/Dialog*/Sheet*` Header/Content/Footer; tipografia e cor em Title/Description/Label; quase tudo em `TableCell`/`TableHead`.
...
| `contrate` | Adota o sistema do `portal` (Pale Brutalism) sem variação — o app nasceu da extração do Projeto α de lá. Contrato próprio é CÓPIA: mudança de sistema visual se faz nos dois | **Zero radius** | `apps/contrate/STYLE_CONTRACT.md` |
...
- **`cursor: pointer` é regra de `@layer base`, não utilitária escrita a mão.** O preflight do Tailwind v4 não devolve mais o ponteiro que o v3 dava a `button`, e os gatilhos do Base UI (menu, aba, opção, combobox, switch) são `div` com `role` — nunca tiveram ponteiro de agente de usuário. Cada `apps/<app>/src/styles.css` carrega a mesma regra `:where(...)`, cobrindo `button`, `summary`, `select`, checkbox/radio/file, o `label` que embrulha um dos dois e os `role` clicáveis, com `:disabled`, `[aria-disabled="true"]` e `[data-disabled]` de fora — e com `[role="combobox"]` excluindo campo editável, porque o Base UI põe esse papel no próprio `<input>` de busca, onde o cursor certo é o de texto. Exceção conhecida: o `apps/docs` tem uma variante própria e mais antiga em `src/styles/app.css`, sem `:where()` e sem guarda de desabilitado — é **dívida, não padrão a copiar**. A lista fica em `:where()` de propósito: especificidade zero, então um `cursor-*` escrito no elemento continua vencendo. Não espalhar `cursor-pointer` em clicável novo — se faltou ponteiro em algo, o lugar de corrigir é a regra base. O caminho inverso é legítimo e é o único: elemento que carrega `role` clicável **sem** ação de clique (gatilho de tooltip que só precisa ser focável, por exemplo) escreve `cursor-default` no próprio elemento.
