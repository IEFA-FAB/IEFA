...
- **Rodar `bun run check` + testes antes de mergear** qualquer PR, e confirmar verde. Typecheck por-arquivo não pega tudo.
  ...
  - **`apps/<app>/.env` faz a suíte local mentir.** Um teste que importa `@/server/*` (e com isso `env.server.ts`, que valida credencial na carga do módulo) passa na máquina de quem tem `.env` e quebra no CI, que não tem. **Nos workspaces de `bun test` a armadilha está fechada**: o script passa `--no-env-file` (Bun 1.4), então a suíte roda com o mesmo env do CI mesmo com o arquivo no disco — não remova a flag. Continua ABERTA no `sisub` e no `assignment-selection`, que rodam vitest e herdam o carregamento automático do Vite: nesses dois, conferir um teste novo da camada server sem o arquivo. A regra melhor segue valendo nos dois casos: não importar `@/server/*` de teste unitário — extrair a função pura para `src/lib/` e testar ali.
