...
- **Hooks do projeto** (`.claude/settings.json`, scripts em `.claude/hooks/`):
  - `guard.ts` recusa editar arquivo gerado, push na `main`, commit que pula hooks, vitest da
    raiz, `supabase db reset` remoto e `migration repair --status reverted`, e pede confirmação
    humana para `db push`/`migration repair` no banco compartilhado. Se ele recusar, siga o caminho que a mensagem indica, sem contornar.
