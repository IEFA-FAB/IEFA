...
- **Lint de Tailwind (`@shadcn/lint` pelo Oxlint)**: `bun run lint:tailwind`, também dentro do `bun run check` e do `lint` de cada app com `components.json` (então roda nos `check-<app>` do CI). Config em `.oxlintrc.tailwind.jsonc` — nome fora do padrão de propósito, o react-doctor adotaria um `.oxlintrc.json`.
  ...
  - **Valor fora da escala (`no-arbitrary-values`) é aviso; dimensão de layout passa.** `text-[10px]`/`text-[11px]` são `text-3xs`/`text-2xs` (tokens do `@theme` de cada app de UI). **Token de tamanho novo entra também no `extendTailwindMerge` do `cn`**: sem isso o tailwind-merge lê `text-3xs` como cor e o descarta diante de um `text-muted-foreground`.
