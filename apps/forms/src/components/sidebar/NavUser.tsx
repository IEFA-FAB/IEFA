					...
					<DropdownMenuTrigger
						render={
							<SidebarMenuButton size="default" className="data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground gap-2">
								...
								<span className="shrink-0 font-mono text-2xs font-semibold uppercase tracking-(--tracking-label) text-muted-foreground">{initials}</span>
								{/* Primeiro nome — só aparece no estado expandido */}
								<span className="flex-1 truncate text-2xs font-medium uppercase tracking-(--tracking-label)">{firstName}</span>
							</SidebarMenuButton>
						}
					/>
