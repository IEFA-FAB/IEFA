...
export const checkDatabaseStatusFn = createServerFn({ method: "GET" }).handler(async (): Promise<{ status: "ok" }> => {
	...
	try {
		const { error } = await getIefaServerClient().from("facilities_pregoeiro").select("id").limit(1).abortSignal(controller.signal)
		if (error) throw new Error(error.message)
		return { status: "ok" }
	} catch (error) {
		if (error instanceof Error && error.name === "AbortError") throw new Error("database_timeout")
		throw error
	} finally {
