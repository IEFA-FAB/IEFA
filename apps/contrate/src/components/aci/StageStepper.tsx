...
export function StageBadge({ stage }: { stage: Stage }) {
	return (
		<Badge variant={stage === "parecer" ? "default" : "outline"} className="text-3xs uppercase tracking-widest">
			{STAGE_LABEL[stage]}
		</Badge>
	)
}
		...
		<ol className="grid grid-cols-4 gap-px border border-border bg-border">
			{STAGE_ORDER.map((step, index) => {
				...
				return (
					...
					>
						<span className="block font-mono text-3xs tabular-nums">{index + 1}</span>
						<span className="block text-xs uppercase tracking-widest">{STAGE_LABEL[step]}</span>
					</li>
				)
			})}
		</ol>
