...
### Agendamento da Produção (`/kitchen/$kitchenId/planning`)
**PlanningBoard** — calendário mensal onde cardápios semanais, eventos e apoios viram o que a cozinha produz em cada dia, e onde entram os ajustes do dia a dia (troca de preparação, substituto de insumo, cardápio de contingência). Fica por último no grupo **Planejamento da Produção** do menu, depois de Cardápios Semanais, Eventos e Apoios.
...
createServerFn({ method: 'GET' | 'POST' })
  .validator(z.object({ ... }))        // validator, NÃO inputValidator (deprecado)
  .handler(async ({ data }) => {
...
- **Supabase**, schema `sisub` como default. O domínio foi dividido por área desde então:
  `core`, `kitchen`, `inventory`, `procurement`, `finance`, `access_control`,
  `nutrition_reference`, `siafi_integration` e `compras_gov_integration`
