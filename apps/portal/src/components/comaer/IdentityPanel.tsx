...
export function IdentityPanel({ input, kind, onChange }: { input: DocumentInput; kind: DocumentKind; onChange: (patch: Partial<DocumentInput>) => void }) {
	...
	// Abre sozinho enquanto falta campo e fecha quando está completo: quem já preencheu tudo
	// não precisa rolar por oito campos estáveis para chegar ao texto, e quem não preencheu
	// não descobre a pendência atrás de um clique.
		...
		<Collapsible defaultOpen={!complete} className="border border-border">
			<CollapsibleTrigger className="group/dados flex w-full items-baseline justify-between gap-3 p-4 text-left outline-none focus-visible:ring-[3px] focus-visible:ring-ring/50">
				...
				<span className="flex items-baseline gap-2 text-label text-muted-foreground">
					...
					<NavArrowDown className="size-4 shrink-0 self-center transition-transform group-aria-expanded/dados:rotate-180" />
				</span>
			</CollapsibleTrigger>
			...
			<CollapsibleContent className="flex flex-col gap-4 px-4 pb-4">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
					<Campo id={field("om")} label="OM expedidora" hint="Nome por extenso, como sai na epígrafe." required>
						<Input
							id={field("om")}
						...
						<>
							...
							{kind.numbering !== "interna" && (
								<Campo
									...
									hint="O contador único da OM, o último número da linha: Ofício nº 34/GAB/255."
								>
									<Input
										id={field("organization")}
					...
					<Campo id={field("date")} label="Data">
						<Input
							id={field("date")}
							type="date"
							value={toDateInputValue(input.date)}
							onChange={(e) => onChange({ date: fromDateInputValue(e.target.value) })}
						/>
					</Campo>
					...
					<Campo id={field("rank")} label="Posto ou graduação" hint="Escolha da lista: em documento externo o posto sai por extenso." required>
