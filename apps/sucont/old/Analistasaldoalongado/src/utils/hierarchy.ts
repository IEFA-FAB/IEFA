...
export const UG_HIERARCHY: Record<string, UgHierarchy> = {
	...
	"120999": { ug: "120999", nome: "MAER - DIF. CAMBIAL", orgaoSuperior: "STN", ods: "STN" },
}

export const getUgHierarchy = (ug: string): UgHierarchy => {
	return UG_HIERARCHY[ug] || { ug, nome: "Desconhecida", orgaoSuperior: "Desconhecido", ods: "Desconhecido" }
}

export const getUniqueOds = (): string[] => {
	const ods = new Set<string>()
	Object.values(UG_HIERARCHY).forEach((item) => ods.add(item.ods))
	return Array.from(ods).sort()
}

export const getUniqueOrgaosSuperiores = (): string[] => {
	const orgaos = new Set<string>()
	Object.values(UG_HIERARCHY).forEach((item) => orgaos.add(item.orgaoSuperior))
