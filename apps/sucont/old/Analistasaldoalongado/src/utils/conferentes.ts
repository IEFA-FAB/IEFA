export const CONFERENTES_MAP: Record<string, string> = {
	"120001": "1S ELIANA",
	"120002": "1S ELIANA",
	"120004": "1S ELIANA",
	"120005": "1S ELIANA",
...
export const getConferente = (ug: string): string => {
	return CONFERENTES_MAP[ug] || "Não Atribuído"
}

export const formatUgWithConferente = (ug: string, nomeUg?: string): string => {
	const conferente = getConferente(ug)
	if (nomeUg) {
		return `${nomeUg} (UG ${ug}) — Conferente: ${conferente}`
	}
	return `UG ${ug} — Conferente: ${conferente}`
}
