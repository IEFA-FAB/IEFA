...
const findSaldoKey = (keys: string[]): string | undefined => {
	...
	if (saldoKeys.length === 0) {
		// Fallback if it doesn't start with saldo but contains it
	...
	// Sort by date extracted from column name (descending)
	saldoKeys.sort((a, b) => {
		...
		reader.onload = async (e) => {
			try {
				...
				if (file.name.endsWith(".csv")) {
					...
					const workbook = XLSX.read(data, { type: "array", raw: true }) // raw: true for performance
