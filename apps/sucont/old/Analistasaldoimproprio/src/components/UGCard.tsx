...
import type { ProcessedRow } from "../App"
...
export const UGCard: React.FC<UGCardProps> = ({ group, type, activeRacFilter }) => {
	...
	let contextMsg = ""
	if (isFocal) {
		contextMsg = `No âmbito do acompanhamento de ${getRacTopic(activeRacFilter)}, identificamos que essa Unidade Gestora apresentou, na data de ${formatDeadlineDate(sendDate)}, ocorrência nas seguintes contas:\n\n${listaContas}`
	} else {
		contextMsg = `Após análise do relatório extraído do Tesouro Gerencial, identificamos que essa Unidade Gestora apresentou, na data de ${formatDeadlineDate(sendDate)}, ocorrência nas seguintes contas:\n\n${listaContas}`
	}
	...
	const baseMsg = baseParts.join("\n\n")

	let assuntoMsg = ""
	if (isFocal) {
		assuntoMsg = `Assunto: Mapeamento Contábil - ${getRacTopic(activeRacFilter)} - ${group.mes}`
	} else {
		assuntoMsg = `Assunto: Mapeamento Contábil - Saldos Transitórios - ${group.mes}`
	}

	const headerMsg = `Mensagem n° ${msgNumber || "___"}/SUCONT-3/${formatMessageDate(sendDate)}\n\n${assuntoMsg}\n\n`
	const fullMessage = headerMsg + baseMsg
				...
				<div className="flex flex-col xl:flex-row gap-6">
					...
					<div className="flex-1 bg-slate-50 rounded-xl p-4 border border-slate-200 flex flex-col min-w-[300px] xl:w-1/2">
						...
						<div className="grid grid-cols-2 gap-3 mb-4">
							<div>
								<label className="block text-xs font-medium text-slate-700 mb-1">Nº da Mensagem</label>
								<input
									type="text"
									value={msgNumber}
							...
							<div>
								<label className="block text-xs font-medium text-slate-700 mb-1">Data de Envio</label>
								<input
									type="date"
									value={sendDate}
							...
							<div className="col-span-2">
								<label className="block text-xs font-medium text-slate-700 mb-1">Tipo de Mensagem</label>
								<select
									value={messageType}
