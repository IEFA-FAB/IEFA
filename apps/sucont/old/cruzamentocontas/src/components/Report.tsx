...
import type { ReportData, UGAnalysis } from "../utils/analyzer"
import { CONFERENTES } from "../utils/conferentes"
...
SUCONT-3 • DIREF • COMAER`
	...
	// Agrupamento gerencial por conferente
	...
	const rankingODS = useMemo(() => {
		...
		return Object.entries(grupos)
			.map(([ods, dados]) => ({ ods, ...dados, percentage: (dados.count / ugsComInconsistencias.length) * 100 }))
	...
	const rankingOrgaoSuperior = useMemo(() => {
		...
		return Object.entries(grupos)
			.map(([orgao, dados]) => ({ orgao, ...dados, percentage: (dados.count / ugsComInconsistencias.length) * 100 }))
			.sort((a, b) => b.count - a.count)
	}, [ugsComInconsistencias])
	...
	return (
		<div className="w-full max-w-6xl mx-auto space-y-8 pb-12">
			...
			<div className="flex p-1 bg-slate-200/50 rounded-2xl w-full max-w-2xl mx-auto border border-slate-200">
				<button
					onClick={() => setSelectedLevel("ESTRATEGICO")}
					className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-bold transition-all ${
						selectedLevel === "ESTRATEGICO" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-white/50"
					}`}
				>
				...
				<button
					onClick={() => setSelectedLevel("TATICO")}
					className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-bold transition-all ${
						selectedLevel === "TATICO" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-white/50"
					}`}
				>
				...
				<button
					onClick={() => setSelectedLevel("OPERACIONAL")}
					className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-bold transition-all ${
						selectedLevel === "OPERACIONAL" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700 hover:bg-white/50"
					}`}
				>
			...
			{/* VISÃO EXECUTIVA FINAL - COM ANÁLISE GERENCIAL */}
						...
						<div className="bg-slate-50 p-5 rounded-xl border border-slate-200">
							...
							<div className="space-y-3">
								...
								<div className="text-xs text-slate-500 mt-2 pt-2 border-t border-slate-200">
									...
									{rankingODS.slice(0, 3).map((ods, _idx) => (
										<div key={ods.ods} className="flex justify-between items-center mt-1">
											<span>{ods.ods}</span>
			...
			{/* ANÁLISE TÁTICA E ESTRATÉGICA DETALHADA */}
			...
			{/* RANKING DE PRIORIDADE E FILTROS */}
							...
							<div className="p-6 bg-slate-50/50 space-y-6">
								...
								<div className="mb-4 p-4 bg-slate-50 border border-slate-200 rounded-lg flex flex-wrap gap-4 items-end">
									<div className="flex flex-col gap-1.5">
										<label className="text-xs font-semibold text-slate-600 uppercase">Nº da Mensagem</label>
										<input
											type="text"
									...
									<div className="flex flex-col gap-1.5">
										<label className="text-xs font-semibold text-slate-600 uppercase">Data da Mensagem</label>
										<input
											type="date"
									...
									<div className="flex flex-col gap-1.5">
										<label className="text-xs font-semibold text-slate-600 uppercase">Tipo de Mensagem</label>
										<select
											value={consolidatedConfig.messageType}
											onChange={(e) => setConsolidatedConfig({ ...consolidatedConfig, messageType: e.target.value as any })}
											className="px-3 py-1.5 border border-slate-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-[#0033A0]"
										>
									...
									{consolidatedConfig.messageType === "COM_PRAZO" && (
										<div className="flex flex-col gap-1.5 ml-2">
											<label className="text-xs font-semibold text-slate-600 uppercase">Data do Prazo</label>
											<input
												type="date"
												value={consolidatedConfig.deadlineDate}
												onChange={(e) => setConsolidatedConfig({ ...consolidatedConfig, deadlineDate: e.target.value })}
												className="px-3 py-1.5 border border-slate-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-[#0033A0]"
											/>
										</div>
									)}
					...
					{filteredUgs.length === 0 ? (
						...
						filteredUgs.map((ug) => (
							<div key={ug.ug} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
								<button onClick={() => toggleUg(ug.ug)} className="w-full px-6 py-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
									<div className="flex items-center gap-6">
										<div className="flex items-center gap-4">
											...
											/>
											<div className="w-12 h-12 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center shrink-0 hidden sm:flex">
												...
												<div className="mb-4 p-4 bg-slate-50 border border-slate-200 rounded-lg flex flex-wrap gap-4 items-end">
													...
													<div className="flex flex-col gap-1.5">
														...
														<select
															...
															onChange={(e) => updateConfig(ug.ug, { messageType: e.target.value as any })}
															className="px-3 py-1.5 border border-slate-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-[#0033A0]"
														>
															<option value="SEM_PRAZO">Padrão (Sem Prazo)</option>
															<option value="COM_PRAZO">Com Prazo</option>
