...
export default function App() {
	...
	const _handleSendMessage = async (predefinedMessage?: string) => {
		const userMsg = predefinedMessage || aiInput.trim()
		...
		try {
			...
			const conciseReports = reports.map((r) => ({
				ug: r.ug,
				conferente: r.conferente,
				totalDiff: r.totalDiff,
				inconsistencias: r.pairs.map((p) => p.question.split(" do ")[0]),
			}))

			const systemInstruction = `Você é o Oráculo SUCONT, um assistente técnico e estratégico especializado em Contabilidade Pública Federal para o Comando da Aeronáutica (COMAER). Sua missão é apoiar a Seção de Acompanhamento Contábil (SUCONT-3.1) na análise de dados, governança financeira e suporte às unidades gestoras.

# 🚨 HIERARQUIA E DESTAQUES CRÍTICOS
- **SETORIAL CONTÁBIL DO COMAER:** Autoridade máxima em normas e fiscalização contábil.
  * 120002 (DIREF)
  * 120701 (DIREF/SUCONT)
  * 120702 (DIREF/SUCONV)
  * 121002 (DIREF - FAer)
- **ÓRGÃO CENTRAL (STN):**
  * 120999 (MAER - DIF. CAMBIAL) – Exclusiva para lançamentos da Secretaria do Tesouro Nacional.

# BASE DE DADOS DE UNIDADES GESTORAS (UGs)
Abaixo está a lista completa de UGs do COMAER com seus respectivos Órgãos Superiores e ODS:
${Object.entries(UG_METADATA)
	.map(([code, info]) => `${code}: ${info.name} (Superior: ${info.superior}, ODS: ${info.ods})`)
	.join("\n")}

# REGRAS DE RESPOSTA
1. Ao identificar uma UG, mencione sempre o Nome Reduzido e o ODS.
2. Priorize a conformidade técnica baseada no MCASP e normas da Setorial.
3. Se houver menção à UG 120999, destaque que o tratamento é diferenciado por ser da STN.
4. Respostas devem ser estruturadas, analíticas e voltadas para a solução de problemas.

Os dados atuais das inconsistências contábeis são:
${JSON.stringify(conciseReports)}

Quando solicitado um "Painel de Risco Contábil do COMAER" ou similar, você deve gerar um panorama consolidado contendo:
- total de inconsistências identificadas
- distribuição por ODS (Órgão de Direção Setorial) e Órgão Superior
- principais contas contábeis envolvidas
- principais questões RAC afetadas

Sempre que possível, calcule indicadores como:
- número total de inconsistências identificadas
- percentual de inconsistências por ODS
- percentual de inconsistências por Órgão Superior
- média de inconsistências por UG
- participação percentual de cada comando no total analisado

Gere automaticamente rankings se solicitado:
- Ranking de ODS por inconsistência
- Ranking de Órgãos Superiores por inconsistência
- Ranking de UGs com maior saldo irregular
- Ranking de contas contábeis com maior ocorrência de inconsistência

Responda sempre em Markdown, de forma clara, profissional e estruturada.`

			const prompt = `${aiMessages.map((m) => `${m.role === "user" ? "Usuário" : "Assistente"}: ${m.text}`).join("\n\n")}\n\nUsuário: ${userMsg}\n\nAssistente:`
			...
			setAiMessages((prev) => [
				...prev,
				{ role: "model", text: "Desculpe, ocorreu um erro ao processar sua solicitação. Verifique se a chave da API está configurada corretamente." },
			])
		} finally {
	...
	const processData = (data: any[][]) => {
		...
		for (const ug in ugs) {
			...
			const chartData: { name: string; description: string; value: number; color: string; question: string }[] = []
			const divergentPairs: any[] = []
			let totalDiff = 0
...
`.trim()
	...
	const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		reader.onload = (evt) => {
			try {
				...
				setError(err.message || "Erro ao processar o arquivo. Certifique-se de que é uma planilha Excel válida.")
			} finally {
	...
	const generateMessageText = useCallback(
		(report: UGReport, type: "com_prazo" | "sem_prazo" | "alerta", prazoDias: string, msgNum: string, msgDateStr: string, customSubject: string) => {
			...
			const _isMixed = (hasPair1 ? 1 : 0) + (hasPair2 ? 1 : 0) + (hasPair3 ? 1 : 0) > 1
			...
			if (isOnlyPair1) {
				...
				closingText = `Por fim, esta Divisão de Acompanhamento Contábil e de Suporte ao Usuário, da Subdiretoria de Contabilidade, permanece à disposição para esclarecer eventuais dúvidas acerca do assunto.`
			} else if (isOnlyPair2) {
...
`
	...
	const copyRacSummary = () => {
		...
		const body = filteredReports
			.map((r) => {
				...
				// Find the specific discrepancy for this RAC question
		...
		if (racFilter !== "all") {
			result = result
				...
				.map((r) => {
					...
					// We need to find which divergencias correspond to these pairs.
					// Since they are indexed 1, 2, 3... in the string, it's a bit tricky.
					// Let's just recalculate them or filter by content.
	...
	return (
		<div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-20">
			{/* Header */}
			<header className="bg-gradient-to-r from-[#003366] via-[#004d99] to-[#003366] text-white shadow-lg border-b-4 border-[#00a8e8] relative overflow-hidden">
				...
				<div className="max-w-7xl mx-auto px-6 py-8 relative z-10">
					<div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
						<div className="flex items-center gap-5">
							<div className="bg-white/10 p-3 rounded-xl border border-white/20 shadow-inner backdrop-blur-sm">
								<Shield className="w-10 h-10 text-[#00a8e8]" />
							</div>
							<div>
								<h1 className="text-3xl font-black tracking-tight flex items-center gap-3">
									ANALISTA SUCONT
									<span className="text-xs font-bold bg-[#00a8e8] text-[#003366] px-2 py-1 rounded uppercase tracking-wider shadow-sm">DIREF</span>
								</h1>
								<p className="text-blue-100 text-sm mt-1 font-medium tracking-wide uppercase">Diretoria de Economia e Finanças da Aeronáutica</p>
							</div>
						</div>

						{/* User Identity Selection */}
						<div className="flex items-center gap-3 bg-white/10 p-2 rounded-xl border border-white/10 backdrop-blur-sm">
							<span className="text-xs font-bold text-blue-100 uppercase ml-2">Identificar-se:</span>
							<div className="flex gap-1">
								<button
									...
									className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${userProfile === null ? "bg-[#00a8e8] text-[#003366] shadow-lg" : "bg-white/5 text-white hover:bg-white/10"}`}
								>
									Todos
								</button>
								{["Jefferson", "Érika", "Eliana", "Pâmela"].map((name) => (
									<button
										...
										className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${userProfile === name ? "bg-[#00a8e8] text-[#003366] shadow-lg" : "bg-white/5 text-white hover:bg-white/10"}`}
									>
										{name}
									</button>
								))}
							...
							<>
								...
								<div className="space-y-8">
									{racFilter !== "all" && filteredReports.length > 0 && (
										<div className="bg-blue-600 text-white px-6 py-4 rounded-xl shadow-md flex items-center justify-between">
											<div className="flex items-center gap-3">
										...
										return (
											<div key={idx} className="bg-white rounded-2xl shadow-md border border-slate-200 overflow-hidden flex flex-col">
												...
												<div className="grid grid-cols-1 lg:grid-cols-12 divide-y lg:divide-y-0 lg:divide-x divide-slate-200">
													{/* Left: Chart Evidence (Simplified) */}
													...
													<div className="p-6 lg:col-span-7 flex flex-col">
														<div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
															<h4 className="text-sm font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2">Mensagem Institucional</h4>

															<div className="flex flex-wrap items-center gap-3">
																<div className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200">
																	<label className="text-xs font-medium text-slate-600 whitespace-nowrap">Nº da Mensagem:</label>
																	<input
																		type="text"
																		placeholder="Ex: 123"
																		value={currentMsgNum}
																		onChange={(e) => setMsgNumbers((prev) => ({ ...prev, [report.ug]: e.target.value }))}
																...
																<div className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200">
																	<label className="text-xs font-medium text-slate-600 whitespace-nowrap">Data:</label>
																	<input
																		type="date"
																		value={currentMsgDate}
																		onChange={(e) => setMsgDates((prev) => ({ ...prev, [report.ug]: e.target.value }))}
																...
																<div className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200">
																	<label className="text-xs font-medium text-slate-600 whitespace-nowrap">Assunto:</label>
																	<input
																		type="text"
																		placeholder="Assunto da mensagem"
																		value={currentSubject}
																		onChange={(e) => setMsgSubjects((prev) => ({ ...prev, [report.ug]: e.target.value }))}
																...
																<div className="flex items-center bg-slate-100 p-1 rounded-lg border border-slate-200">
																	<button
																		onClick={() => setMessageTypes((prev) => ({ ...prev, [report.ug]: "com_prazo" }))}
																	...
																	<button
																		onClick={() => setMessageTypes((prev) => ({ ...prev, [report.ug]: "sem_prazo" }))}
																	...
																	<button
																		onClick={() => setMessageTypes((prev) => ({ ...prev, [report.ug]: "alerta" }))}
																...
																{msgType === "com_prazo" && (
																	<div className="flex items-center gap-2 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200">
																		...
																		<label className="text-xs font-medium text-slate-600 whitespace-nowrap">Prazo:</label>
																		<input
																			type="date"
																			value={currentPrazo}
																			onChange={(e) => setPrazos((prev) => ({ ...prev, [report.ug]: e.target.value }))}
																			className="px-2 py-1 text-sm border border-slate-300 rounded focus:ring-1 focus:ring-[#00a8e8] focus:border-[#00a8e8] outline-none bg-white"
																		/>
																	</div>
																)}
											...
											<div className="h-72">
												<ResponsiveContainer width="100%" height="100%">
													<BarChart data={managerialData.topUGs} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
														...
														<XAxis type="number" tickFormatter={(value) => `R$ ${(value / 1000000).toFixed(1)}M`} stroke="#64748b" fontSize={12} />
											...
											<div className="h-72">
												<ResponsiveContainer width="100%" height="100%">
													<PieChart>
														...
														<Tooltip
															formatter={(value: number) => [`${value} UGs`, "Ocorrências"]}
															contentStyle={{ borderRadius: "8px", border: "none", boxShadow: "0 4px 6px -1px rgb(0 0 0 / 0.1)" }}
														/>
			...
			<footer className="max-w-7xl mx-auto px-6 pb-8 text-center">
				<p className="text-xs text-slate-400 leading-relaxed max-w-4xl mx-auto">
					...
					Carlos José Rodrigues, com supervisão do Cel Int Eduardo de Oliveira Silva (Chefe da SUCONT-3) e desenvolvimento técnico do 1º Ten QOAp CCO Jefferson
					Luís Reis Alves (Chefe da SUCONT-3.1).
				</p>
			</footer>
		</div>
	)
