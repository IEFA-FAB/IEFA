...
import { Maximize2, Minimize2, X, Filter, Info } from 'lucide-react';
import { FinancialRecord, AccountGroup, RiskLevel } from '../types';
...
interface ChartWrapperProps {
  ...
  showRiskFilter?: boolean; // New prop
...
export const ChartWrapper: React.FC<ChartWrapperProps> = ({
  ...
  showRiskFilter = false, // Default to false
  hierarchyLevel,
  hierarchyFilter,
  className = ''
}) => {
  ...
  const [localRisk, setLocalRisk] = useState<string>('TODOS'); // New state

  // Update local state if defaults change
  useEffect(() => { setLocalMonth(defaultMonth); }, [defaultMonth]);
  useEffect(() => { setLocalGroup(defaultGroup); }, [defaultGroup]);
      ...
      {showRiskFilter && (
        <div className="flex items-center gap-1">
          ...
          <div className="group relative">
            ...
            <div className={`absolute top-full right-0 mt-2 w-72 p-3 rounded-lg border shadow-2xl z-[60] invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-200
            ...
            >
              ...
              <div className="space-y-1 text-[9px]">
                <div className="flex gap-2"><span className="font-bold text-emerald-500">Baixo Risco</span> <span>→ divergência pequena e pouco recorrente</span></div>
                <div className="flex gap-2"><span className="font-bold text-yellow-500">Médio Risco</span> <span>→ divergência moderada ou recorrência moderada</span></div>
                <div className="flex gap-2"><span className="font-bold text-orange-500">Alto Risco</span> <span>→ divergência relevante ou recorrente</span></div>
                <div className="flex gap-2"><span className="font-bold text-red-500">Crítico</span> <span>→ divergência muito elevada e persistente</span></div>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <>
      {/* Placeholder to keep space in the grid when expanded */}
      ...
      {/* The actual card */}
      <div className={`rounded-lg border shadow-sm flex flex-col overflow-visible transition-all duration-300
      ...
      `}>
        ...
        <div className={`flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-2 flex-shrink-0 ${isExpanded ? 'border-b pb-4' : ''}`}>
          <h2 className={`font-semibold ${isDarkMode ? 'text-slate-200' : 'text-slate-800'} ${isExpanded ? 'text-xl' : 'text-lg'}`}>
            {title}
          </h2>
        ...
        <div className="flex-1 relative w-full overflow-hidden flex flex-col">
          <div className="w-full h-full relative min-h-0 flex-1">
            {children(filteredData, isExpanded)}
          </div>
        </div>
      </div>
    </>
