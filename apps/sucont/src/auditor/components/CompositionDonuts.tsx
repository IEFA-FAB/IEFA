...
export const CompositionDonuts: React.FC<CompositionDonutsProps> = ({ data, isDarkMode }) => {
	const stats = useMemo(() => {
		...
		const compositionData = [
			{ name: "SIAFI", value: totalSiafi, color: "#3b82f6" },
			{ name: "SILOMS", value: totalSiloms, color: "#10b981" },
		]

		const detailData = [
			{ name: "BMP", value: groupDiffs[AccountGroup.BMP], color: "#dc2626" },
			{ name: "CONSUMO", value: groupDiffs[AccountGroup.CONSUMO], color: "#2563eb" },
			{ name: "INTANGÍVEL", value: groupDiffs[AccountGroup.INTANGIVEL], color: "#059669" },
		]
		...
		<div className="flex flex-col gap-4 h-full">
			<div className={`flex-1 flex flex-col p-4 rounded-lg border ${isDarkMode ? "bg-[#0f172a]/40 border-slate-800" : "bg-white border-slate-100"} shadow-sm`}>
				...
				<div className="flex-1 relative">
					<ResponsiveContainer width="100%" height="100%">
						<PieChart>
							<Pie data={stats.compositionData} cx="50%" cy="50%" innerRadius="60%" outerRadius="80%" paddingAngle={5} dataKey="value" stroke="none">
					...
					<div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
						...
						<span className="text-[8px] font-bold uppercase tracking-tighter" style={{ color: subTextColor }}>
							TOTAL
						</span>
					</div>
			...
			<div className={`flex-1 flex flex-col p-4 rounded-lg border ${isDarkMode ? "bg-[#0f172a]/40 border-slate-800" : "bg-white border-slate-100"} shadow-sm`}>
				...
				<div className="flex-1 relative">
					<ResponsiveContainer width="100%" height="100%">
						<PieChart>
							<Pie data={stats.detailData} cx="50%" cy="50%" innerRadius="60%" outerRadius="80%" paddingAngle={5} dataKey="value" stroke="none">
