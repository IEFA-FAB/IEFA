...
describe("varredura", () => {
	...
	it("encontra os caminhos conhecidos até o modelo", () => {
		expect(DIRECT_ADAPTER.map((s) => s.path).sort()).toEqual([
			"routes/api/auditor/report.post.ts",
			"routes/api/chat/stream.post.ts",
			"routes/api/sacdgc/analyze.post.ts",
		])
...
describe.each(DIRECT_ADAPTER.map((s) => [s.path, s] as const))("adapter direto — %s", (_path, source) => {
	...
	it("exige sessão e permissão de um módulo do sucont", () => {
...
describe.each(VIA_GUARD_MODULE.map((s) => [s.path, s] as const))("via ai.server — %s", (_path, source) => {
	it("exige conta com grant de um módulo do sucont", () => {
