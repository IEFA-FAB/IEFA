...
/**
 * Sidebar do shadcn, portada do sisub e reduzida ao que o hub usa.
 *
 * Duas diferenças em relação ao shadcn de origem, herdadas do sisub:
 *
 * 1. **Sem Radix.** O `asChild` do Radix vira `render` + `useRender`/`mergeProps`
 *    do Base UI — é a regra do monorepo, e é o que permite `SidebarMenuButton`
 *    virar um `<Link>` do TanStack sem aninhar botão dentro de âncora.
 * 2. **Estado persistido em cookie**, lido no `beforeLoad` da rota raiz. O SSR já
 *    manda o HTML com a sidebar no estado certo; sem isso ela renderiza aberta no
 *    servidor e salta para fechada na hidratação.
 *
 * Uma diferença em relação ao sisub: o modo é `offcanvas`, o nome do shadcn. No
 * sisub ele aparece como `offExamples` — consistente lá, mas é o token errado.
 */
...
function SidebarProvider({
	...
	const setOpen = React.useCallback(
		(value: boolean | ((value: boolean) => boolean)) => {
			...
			// biome-ignore lint/suspicious/noDocumentCookie: o estado da sidebar precisa sobreviver ao F5 e ser legível no SSR
	...
	// Ctrl/Cmd+B alterna — o mesmo atalho do sisub.
	React.useEffect(() => {
...
function Sidebar({
			...
			<Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
				...
				>
					<SheetHeader className="sr-only">
						<SheetTitle>Menu do hub</SheetTitle>
						<SheetDescription>Navegação entre as telas do SUCONT-4 HUB.</SheetDescription>
					</SheetHeader>
	...
	return (
		<div
			...
			data-slot="sidebar"
		>
			{/* Reserva a largura no fluxo: a sidebar em si é `fixed`. */}
			<div
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			...
			className={cn("gap-0 flex min-h-0 flex-1 flex-col overflow-auto group-data-[collapsible=icon]:overflow-hidden", className)}
			{...props}
		/>
	)
}
...
function SidebarMenuSkeleton({ className, showIcon = false, ...props }: React.ComponentProps<"div"> & { showIcon?: boolean }) {
	// Largura entre 50% e 90%, sorteada uma vez: linhas de tamanho igual parecem
	// uma tabela, não texto carregando.
