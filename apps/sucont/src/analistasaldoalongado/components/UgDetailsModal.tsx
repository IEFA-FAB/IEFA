...
export function UgDetailsModal({ ugData, onClose, initialRacFilter }: UgDetailsModalProps) {
	...
	}, [ugData, messageNumber, messageDate, messageType, deadlineDate, selectedRacFilter])
	...
	const filteredAndSortedOcorrencias = useMemo(() => {
		...
		return filtered.sort((a, b) => (sortDirection === "asc" ? a.saldo - b.saldo : b.saldo - a.saldo))
	}, [ugData, searchTerm, sortDirection, selectedRacFilter])
			...
			<div className="w-full max-w-7xl h-full max-h-[90vh] bg-muted/50 rounded-xl shadow-2xl flex flex-col overflow-hidden border border-border">
				...
				<div className="flex items-center justify-between px-6 py-4 bg-card border-b border-border shrink-0">
					<div className="flex items-center gap-4">
						<div className="w-12 h-12 rounded-xl bg-fab-100 flex items-center justify-center text-fab-700 text-heading">UG</div>
						<div>
				...
				<div className="flex-1 overflow-hidden flex flex-col lg:flex-row">
					...
					<div className="flex-1 flex flex-col border-r border-border bg-card overflow-hidden">
						<div className="p-4 border-b border-border bg-muted/50 flex flex-col sm:flex-row justify-between items-center gap-4">
							...
							<div className="relative w-full sm:w-64">
								...
								<Input
									...
									className="pl-9 pr-4 py-1.5 text-body border-border rounded-lg focus-visible:ring-fab-500 focus-visible:border-fab-500 bg-card text-foreground"
									value={searchTerm}
						...
						{racQuestions.length > 0 && (
							<div className="px-4 py-3 bg-fab-50 border-b border-fab-100 flex items-start gap-3">
								...
								<div>
									<h4 className="text-subheading text-fab-900 mb-1">Informação para Análise (Controle Interno SUCONT-3)</h4>
									<p className="text-caption text-fab-800 mb-2">
									...
									{filteredAndSortedOcorrencias.map((occ, idx) => (
										<tr key={idx} className="hover:bg-muted/50 transition-colors">
											...
											<td className="px-4 py-3">
												<span className="inline-flex items-center px-2 py-0.5 rounded text-hint bg-fab-50 text-fab-700 border border-fab-100 whitespace-nowrap">
													{occ.status}
												</span>
											</td>
										</tr>
					...
					{/* Right Side - Message */}
					<div className="w-full lg:w-[450px] flex flex-col bg-muted/50 shrink-0 border-l border-border">
						<div className="p-4 border-b border-border bg-card flex items-center justify-between">
							...
							<Button
								type="button"
								onClick={handleCopy}
								className="gap-2 px-3 py-1.5 text-caption text-white bg-fab-600 border-transparent rounded-lg hover:bg-fab-700 transition-colors shadow-sm"
							>
								{copied ? (
									<>
