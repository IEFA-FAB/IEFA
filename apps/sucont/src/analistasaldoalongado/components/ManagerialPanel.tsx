...
export function ManagerialPanel({ data }: ManagerialPanelProps) {
	...
	const getDynamicAccountName = (account: string) => {
		...
		if (hardcodedName !== account) return hardcodedName
	...
	const filteredSaldoTotal = useMemo(() => filteredData.reduce((acc, curr) => acc + curr.saldo_total, 0), [filteredData])
	...
	const concentracaoRisco = useMemo(() => {
		...
		return filteredSaldoTotal > 0 ? ((top5Saldo / filteredSaldoTotal) * 100).toFixed(1) : "0.0"
							...
							<BarChart
								...
								onClick={(state) => {
									...
									if (s.activePayload && s.activePayload.length > 0) {
										setSelectedOdsDetails(s.activePayload[0].payload.name ?? null)
									}
								}}
						...
						<div className="p-6 overflow-y-auto">
							...
							<div className="space-y-3">
								{ugsInSelectedOds.map((ug) => (
									<div key={ug.ug} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border border-border bg-muted/50 gap-2">
										<div>
										...
										<span className="font-bold text-foreground text-right">{formatCurrency(ug.saldo_total)}</span>
									</div>
								))}
