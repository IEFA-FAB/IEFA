	...
	return (
		<div className="space-y-6">
			...
			{showCreateForm && (
				<div className="p-6 border rounded-lg bg-card space-y-4">
					...
					<div className="grid md:grid-cols-3 gap-4">
						...
						<div className="space-y-2">
							<label className="text-sm font-medium">Edição/Número</label>
							<Input type="number" placeholder="Ex: 1" />
						</div>
						<div className="space-y-2">
			...
			{volumes.length > 0 ? (
						...
						>
							...
							<div className="space-y-2 mb-4">
								<div className="flex items-center gap-2 text-sm text-muted-foreground">
									...
									Publicado em{" "}
									{new Date(volume.published_date).toLocaleDateString("pt-BR")}
								</div>
				...
				<div className="flex flex-col items-center justify-center py-16 px-4 text-center border rounded-lg bg-card">
					...
					<h3 className="text-lg font-semibold mb-2">
						Nenhum volume criado ainda
					</h3>
					<p className="text-muted-foreground max-w-md mb-6">
						Crie volumes e edições para organizar os artigos publicados do seu
						periódico.
					</p>
					<Button onClick={() => setShowCreateForm(true)}>
						<Plus className="size-4 mr-2" />
			...
			<div className="p-4 bg-muted rounded-lg">
				...
				<p className="text-sm text-muted-foreground">
					Os volumes e edições ajudam a organizar os artigos publicados
					cronologicamente. Cada artigo deve ser atribuído a um volume e edição
					específicos no momento da publicação.
				</p>
			</div>
		</div>
	);
