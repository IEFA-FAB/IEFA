...
export const publishedArticlesQueryOptions = (filters?: {
	limit?: number;
	offset?: number;
}) =>
	queryOptions({
...
export const editorialDashboardQueryOptions = (filters?: {
	status?: string;
	limit?: number;
}) =>
	queryOptions({
...
export function useCreateArticleAuthors() {
	...
	return useMutation({
		mutationFn: (authors: Partial<ArticleAuthor>[]) =>
			createArticleAuthors(authors),
		onSuccess: (data) => {
			if (data.length > 0) {
...
export function useCreateArticleVersion() {
	...
	return useMutation({
		mutationFn: (version: Partial<ArticleVersion>) =>
			createArticleVersion(version),
		onSuccess: (data) => {
			queryClient.invalidateQueries({
