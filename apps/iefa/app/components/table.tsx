...
function useFacilitiesQuery() {
    return useQuery<Facilidades_pregoeiro[]>({
        ...
        queryFn: async () => {
            const { data, error } = await supabaseApp.from("facilities_pregoeiro").select("*");
    ...
    return [
        {
            ...
            cell: ({ row }) => (
                <div className="capitalize whitespace-pre-wrap wrap-break-word">{row.getValue("phase")}</div>
            ),
            enableHiding: true,
            enableSorting: true,
        },
