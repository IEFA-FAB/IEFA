...
const FacilidadesTableLazy = lazy(() => import("@/components/table").then((m) => ({ default: m.FacilidadesTable })));
...
export function meta() {
    return [{ title: "Facilidades do Pregoeiro" }, { name: "description", content: "Suite de Soluções do IEFA" }];
}
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (isEdit && initial?.id) {
                ...
                const { error } = await supabaseApp.from("facilities_pregoeiro").insert(payload);
                if (error) throw error;
            }
...
export default function Pregoeiro() {
    ...
    const handleChange = (key: keyof FacilidadesTableProps) => (e: React.ChangeEvent<HTMLInputElement>) =>
