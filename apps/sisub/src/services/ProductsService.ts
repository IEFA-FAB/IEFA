 ...
 */
export async function fetchProductItems(productId?: string): Promise<ProductItem[]> {
	let query = supabase
		.from("product_item")
		.select("*")
		.is("deleted_at", null)
...
export async function createFolder(payload: FolderInsert): Promise<Folder> {
	const { data, error } = await supabase.from("folder").insert(payload).select().single()

	if (error) throw new Error(error.message)
	return data
}

export async function updateFolder(id: string, payload: FolderUpdate): Promise<Folder> {
	const { data, error } = await supabase
		.from("folder")
		.update(payload)
		.eq("id", id)
...
export async function createProductItem(payload: ProductItemInsert): Promise<ProductItem> {
	const { data, error } = await supabase.from("product_item").insert(payload).select().single()
