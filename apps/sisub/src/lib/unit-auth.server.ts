...
export async function requireUnitScope(
	...
	if (unitId != null) return requireAuthWithPermission("unit", level, { type: "unit", id: unitId }, assurance)
	// Sem unidade resolvível: só permissão GLOBAL passa — permissão escopada em
	// outra unidade não pode alcançar dados sem escopo.
	const ctx = await requireAuthWithPermission("unit", level, undefined, assurance)
