...
describeSupabaseIntegration("recipes operations (regressão)", () => {
	...
	test("createRecipe insere receita global + ingredientes e retorna a linha", async () => {
		...
		const recipe = await createRecipe(client, ctx, {
	...
	test("fetchRecipe retorna receita com ingredientes aninhados (shape)", async () => {
		...
		const recipe = await fetchRecipe(client, ctx, { recipeId })
	...
	test("listRecipes faz dedup por família mantendo a maior versão", async () => {
		if (!reachable || !seeder) return
		const v1 = await createRecipe(client, ctx, { name: uid("[TEST] Família "), portionYield: 100, kitchenId: null })
		seeder.track("recipes", v1.id)
		const v2 = await createRecipeVersion(client, ctx, {
			name: v1.name,
			portionYield: 120,
			kitchenId: null,
			baseRecipeId: v1.id,
		...
		const recipes = await listRecipes(client, ctx, { kitchenId: null })
	...
	test("deleteRecipe (soft) some da listagem default e reaparece com includeDeleted; restoreRecipe reverte", async () => {
		...
		await deleteRecipe(client, ctx, { id: recipeId })
		const visivel = await listRecipes(client, ctx, { kitchenId: null })
		expect(visivel.map((r) => r.id)).not.toContain(recipeId)

		const comLixeira = await listRecipes(client, ctx, { kitchenId: null, includeDeleted: true })
		expect(comLixeira.map((r) => r.id)).toContain(recipeId)

		await restoreRecipe(client, ctx, { id: recipeId })
		const restaurada = await listRecipes(client, ctx, { kitchenId: null })
	...
	test("listRecipeVersions retorna a família ordenada por version asc", async () => {
		...
		const versions = await listRecipeVersions(client, ctx, { recipeId: v1 })
	...
	test("renameRecipe altera o nome in-place (write→read)", async () => {
		...
		await renameRecipe(client, ctx, { id: recipeId, name: novoNome })
		const recipe = await fetchRecipe(client, ctx, { recipeId })
	...
	test("listRecipeMenuUsage retorna IDs de receitas usadas em template semanal", async () => {
		...
		const usage = await listRecipeMenuUsage(client, ctx)
