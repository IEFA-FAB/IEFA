...
describeSupabaseIntegration("purchase-item operations (regressão)", () => {
	...
	test("fetchPurchaseItems exclui deleted_at e filtra por search (ilike)", async () => {
		...
		const results = await fetchPurchaseItems(client, ctx, { search: tag })
	...
	test("createPurchaseItem → fetchPurchaseItem (round-trip com junção aninhada)", async () => {
		if (!reachable || !seeder) return
		const created = await createPurchaseItem(client, ctx, { payload: { description: uid("[TEST] PI ") } })
		...
		const fetched = await fetchPurchaseItem(client, ctx, { id: created.id })
	...
	test("updatePurchaseItem persiste e deletePurchaseItem é soft (some da listagem)", async () => {
		...
		const updated = await updatePurchaseItem(client, ctx, { id, payload: { description: `${tag} v2` } })
		expect(updated.description).toBe(`${tag} v2`)
		expect(updated.updated_at).toBeTruthy()

		await deletePurchaseItem(client, ctx, { id })
		const remaining = await fetchPurchaseItems(client, ctx, { search: tag })
	...
	test("fetchIngredientPurchaseItems achata o vínculo (link_id, conversion_factor, is_default) e ignora purchase_item deletado", async () => {
		...
		const rows = await fetchIngredientPurchaseItems(client, ctx, { ingredientId })
	...
	test("setDefaultPurchaseItemIngredient torna exclusivo o is_default", async () => {
		...
		await setDefaultPurchaseItemIngredient(client, ctx, { purchaseItemId, id: linkB })

		const links = await fetchPurchaseItemIngredients(client, ctx, { purchaseItemId })
	...
	test("upsertPurchaseItemIngredient resolve conflito (purchase_item_id, ingredient_id)", async () => {
		...
		const purchaseItemId = await seeder.seedPurchaseItem()
		const ingredientId = await seeder.seedIngredient()
		const first = await upsertPurchaseItemIngredient(client, ctx, {
		...
		const second = await upsertPurchaseItemIngredient(client, ctx, {
		...
		await deletePurchaseItemIngredient(client, ctx, { id: second.id })
		const after = await fetchPurchaseItemIngredients(client, ctx, { purchaseItemId })
