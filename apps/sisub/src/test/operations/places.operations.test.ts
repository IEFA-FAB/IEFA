...
describeSupabaseIntegration("places operations (regressão)", () => {
	...
	test("listAllMessHalls inclui o rancho semeado com o shape esperado", async () => {
		...
		const all = await listAllMessHalls(client, ctx)
	...
	test("fetchMessHallByCode + fetchMessHallIdByCode resolvem por code", async () => {
		...
		const byCode = await fetchMessHallByCode(client, ctx, { code })
		expect(byCode).toMatchObject({ id, code, unit_id: unitId })

		const idByCode = await fetchMessHallIdByCode(client, ctx, { code })
		expect(idByCode).toBe(id)

		// code vazio → null sem query
		expect(await fetchMessHallIdByCode(client, ctx, { code: "" })).toBeNull()
	})
	...
	test("fetchPlacesGraph retorna { units, kitchens, messHalls } como arrays", async () => {
		...
		const graph = await fetchPlacesGraph(client, ctx)
	...
	test("updatePlacesEntity (mess_hall) renomeia e persiste", async () => {
		...
		const res = await updatePlacesEntity(client, ctx, { entityType: "mess_hall", id, display_name: newName, code })
		expect(res).toEqual({ ok: true })

		const after = await fetchMessHallByCode(client, ctx, { code })
	...
	test("addOtherPresence + fetchOtherPresencesCount contam presenças avulsas", async () => {
		...
		expect(await fetchOtherPresencesCount(client, ctx, { date, meal, messHallId })).toBe(0)
		await addOtherPresence(client, ctx, { adminId, date, meal, messHallId })
		expect(await fetchOtherPresencesCount(client, ctx, { date, meal, messHallId })).toBe(1)
	})
	...
	test("resolveDisplayName cai para o email quando não há dado militar", async () => {
		...
		const name = await resolveDisplayName(client, ctx, { userId })
