...
describeSupabaseIntegration("permissions operations (regressão)", () => {
	...
	test("listEffectiveUserPermissions injeta diner implícito e descarta deny (level 0)", async () => {
		...
		const perms = await listEffectiveUserPermissions(client, { userId })
	...
	test("CRUD admin: create → fetch (ordenado por module) → update → delete", async () => {
		...
		const created = await createUserPermission(client, ctx, { userId, module: "kitchen", level: 1, mess_hall_id: null, kitchen_id: null, unit_id: null })
		expect(created).toEqual({ success: true })

		const rows = await fetchUserPermissionsAdmin(client, ctx, { userId })
		expect(rows).toHaveLength(1)
		const permId = rows[0].id
		expect(rows[0].module).toBe("kitchen")
		expect(rows[0].level).toBe(1)

		await updateUserPermission(client, ctx, { permissionId: permId, level: 2, mess_hall_id: null, kitchen_id: null, unit_id: null })
		const afterUpdate = await fetchUserPermissionsAdmin(client, ctx, { userId })
		expect(afterUpdate[0].level).toBe(2)

		await deleteUserPermission(client, ctx, { permissionId: permId })
		const afterDelete = await fetchUserPermissionsAdmin(client, ctx, { userId })
	...
	test("searchUsersByEmail encontra por ilike e devolve { id, email, nrOrdem }", async () => {
		...
		const results = await searchUsersByEmail(client, ctx, { email: tag })
