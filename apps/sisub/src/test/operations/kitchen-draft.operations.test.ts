...
describeSupabaseIntegration("kitchen-draft operations (regressão)", () => {
	...
	test("createKitchenDraft cria com status 'pending' e insere selections", async () => {
		...
		const draft = await createKitchenDraft(client, ctx, {
			kitchenId,
	...
	test("fetchKitchenDrafts ordena por created_at desc; fetchPendingDraft só retorna 'sent'", async () => {
		...
		const first = await createKitchenDraft(client, ctx, { kitchenId, title: uid("[TEST] D1 "), notes: undefined, selections: [] })
		seeder.track("kitchen_ata_draft", first.id)
		const second = await createKitchenDraft(client, ctx, {
			kitchenId,
			title: uid("[TEST] D2 "),
			notes: undefined,
		...
		// nenhum 'sent' ainda
		expect(await fetchPendingDraft(client, ctx, { kitchenId })).toBeNull()

		await sendKitchenDraft(client, ctx, { draftId: second.id })
		const pending = await fetchPendingDraft(client, ctx, { kitchenId })
	...
	test("updateKitchenDraft substitui selections (delete-all + re-insert)", async () => {
		...
		await updateKitchenDraft(client, ctx, {
			draftId: draft.id,
