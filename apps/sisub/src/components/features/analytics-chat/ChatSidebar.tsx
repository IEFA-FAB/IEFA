import { MessageSquarePlus, MoreHorizontal, Pencil, Sparkles, Trash2 } from "lucide-react"
import { useCallback, useMemo, useRef, useState } from "react"
import { Button } from "@/components/ui/button"
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from "@/components/ui/dropdown-menu"
import { ScrollArea } from "@/components/ui/scroll-area"
import { useChatSessions, useDeleteChatSession, useRenameChatSession } from "@/hooks/data/useAnalyticsChatHistory"
import { cn } from "@/lib/cn"
import type { ChatSession } from "@/types/domain/analytics-chat"
...
function SessionItem({
	...
	return (
		// biome-ignore lint/a11y/useSemanticElements: contains interactive children (input, dropdown)
		<div
			className={cn(
...
// ── Mobile full-screen session list (WhatsApp-style) ────────────────────────
...
export function MobileChatList({ onSelectSession, onNewChat }: MobileChatListProps) {
	const { data: sessions = [], isLoading } = useChatSessions()
	const deleteMutation = useDeleteChatSession()
				...
				{isLoading ? (
					...
					<div className="px-2 pb-4">
						{sessions.map((session) => (
							// biome-ignore lint/a11y/useSemanticElements: contains interactive children (dropdown button)
							<div
								key={session.id}
