...
import { formatDinerLoad, formatHeadcount, formatRatio, NOTE_KIND_LABELS, NOTE_KIND_VARIANT } from "@/components/features/workforce/labels"
...
export function WorkforceMatrixTable({ matrix, canEdit, queryKey }: WorkforceMatrixTableProps) {
	...
	function commit(rancho: WorkforceRanchoWire) {
		...
		save.mutate({
			...
			entries,
			declaredTotal: total === "" ? null : Number(total),
		})
	}
				...
				<TableHeader>
					<TableRow>
						...
						<TableHead className="text-right">
							<Tooltip>
								<TooltipTrigger render={<span>Comensais/militar</span>} />
								<TooltipContent>Comensais por dia por militar disponível, no mês da competência</TooltipContent>
							</Tooltip>
						</TableHead>
						...
						return (
							<React.Fragment key={rancho.ranchoId}>
								<TableRow className={cn(!rancho.answered && "bg-muted/40")}>
									...
									<TableCell className="text-right tabular-nums">{formatDinerLoad(rancho.diners_per_worker)}</TableCell>
