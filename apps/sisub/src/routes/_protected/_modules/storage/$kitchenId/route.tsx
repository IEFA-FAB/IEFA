...
export const Route = createFileRoute("/_protected/_modules/storage/$kitchenId")({
	beforeLoad: async ({ context, params, preload }) => {
		const kitchenId = Number(params.kitchenId)
		requirePermission({ context, preload }, "storage", 1, { type: "kitchen", id: kitchenId })
		...
		const kitchens = expectArray<KitchenWithUnit>(
			...
			{ source: "fetchKitchensFn", route: "storage/$kitchenId" }
		)

		const kitchen = kitchens.find((k) => k.id === kitchenId)
		if (!kitchen) throw redirect({ to: "/storage", replace: true })
