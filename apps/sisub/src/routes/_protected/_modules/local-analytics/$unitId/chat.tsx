...
function LocalAnalyticsChatPage() {
	...
	const config = useMemo(() => getLocalAnalyticsChatConfig(unitId), [unitId])

	const handleSelectSession = useCallback(
		(id: string) => {
			void navigate({ to: "/local-analytics/$unitId/chat", params: { unitId: unitIdStr }, search: { session: id } })
		},
		[navigate, unitIdStr]
	)
	...
	const handleSessionCreated = useCallback(
		(id: string) => {
			void navigate({ to: "/local-analytics/$unitId/chat", params: { unitId: unitIdStr }, search: { session: id }, replace: true })
		},
		[navigate, unitIdStr]
	)
