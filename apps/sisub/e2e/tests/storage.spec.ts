...
test.describe("Storage — módulo de estoque", () => {
  test("hub /storage lista cozinhas para seleção de escopo", async ({
    authenticatedPage,
  }) => {
    ...
    await expect(
      authenticatedPage.getByText(/Selecionar Cozinha/i).first(),
  ...
  test("dashboard mostra os cards de resumo do ledger", async ({
    authenticatedPage,
  }) => {
    await authenticatedPage.goto(`/storage/${KITCHEN_ID}/dashboard`);
    await expect(
      authenticatedPage.getByText(/Itens em estoque/i).first(),
    ).toBeVisible({ timeout: 20_000 });
    await expect(
      authenticatedPage.getByText(/Vencendo em 30 dias/i).first(),
    ).toBeVisible();
  });

  test("relatórios MCASP mostram balancete e fechamentos", async ({
    authenticatedPage,
  }) => {
    await authenticatedPage.goto(`/storage/${KITCHEN_ID}/reports`);
    await expect(
      authenticatedPage.getByText(/Balancete/i).first(),
    ).toBeVisible({ timeout: 20_000 });
    await expect(
      authenticatedPage.getByText(/Empenho × Liquidação/i).first(),
    ).toBeVisible();
  });
});
