...
test.describe("Auth — fluxo de login via UI", () => {
	...
	test("login com credenciais válidas redireciona para /hub", async ({ page }) => {
		const email = process.env.E2E_TEST_USER_EMAIL
		const password = process.env.E2E_TEST_USER_PASSWORD

		// `test.skip` não estreita o tipo: é o early return que garante ao compilador
		// (e ao leitor) que daqui pra baixo as credenciais existem.
		if (!email || !password) {
	...
	test("login com credenciais inválidas exibe mensagem de erro e permanece em /auth", async ({ page }) => {
		await page.goto("/auth")
		await expect(page.locator("#login-email")).toBeVisible()
		await waitForReactHydration(page, "#login-email")
	...
	test("rota protegida /hub redireciona para /auth quando não autenticado", async ({ page }) => {
