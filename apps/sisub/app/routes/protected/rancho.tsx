...
import { createEmptyDayMeals, type DayMeals, formatDate, getDayOfWeek, isDateNear } from "~/utils/RanchoUtils";
...
export function meta() {
    return [{ title: "Previsão SISUB" }, { name: "description", content: "Faça sua previsão" }];
}
...
export default function Rancho(): JSX.Element {
    ...
    const weekdayTargets = useMemo(
        () => dates.filter((date) => isWeekday(date) && !isDateNear(date, NEAR_DATE_THRESHOLD)),
        [dates],
    );
    ...
    const dayCardsProps = useMemo(() => {
        return computedData.cardData.map(({ date, daySelections, dayMessHallCode }) => {
    ...
    const applyMealTemplateToAll = useCallback(
        async (template: DayMeals, options: { mode: "fill-missing" | "override" }): Promise<void> => {
            ...
            try {
                ...
                targetDates.forEach((date) => {
                    ...
                    // UI (CODE) -> Backend (ID)
                    const codeForDay =
                        (dayMessHalls[date] && dayMessHalls[date] !== "" ? dayMessHalls[date] : defaultMessHallCode) ||
                        "";
                    const idForDay =
