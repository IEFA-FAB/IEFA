...
export function meta() {
    return [{ title: "Lista de Atualizações" }, { name: "description", content: "Veja o que mudou no sistema" }];
}
...
const TagBadge = React.memo(function TagBadge({ tag }: { id: string; tag: string }) {
    const key = (tag ?? "").toLowerCase();
    const tone = TAG_TONE[key] ?? toneBadge("muted");
    return (
        <span className={`inline-flex items-center text-xs font-medium px-2 py-0.5 rounded-full border ${tone}`}>
            {tag}
        </span>
    );
});

const ChangelogCard = React.memo(function ChangelogCard({ entry }: { entry: ChangelogEntry }) {
            ...
            <Card className="py-12">
                <div className="container mx-auto px-4 text-center">
                    ...
                    <p className="text-[hsl(var(--primary-foreground))]/80 max-w-2xl mx-auto mb-6">
                        Ajude a melhorar o SISUB: envie sugestões, correções e novas funcionalidades diretamente pelo
                        GitHub.
                    </p>
                    <a
