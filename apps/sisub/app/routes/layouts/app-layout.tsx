...
export const queryKeys = {
    userNrOrdem: (userId: string | null | undefined) => ["user", userId, "nrOrdem"] as const,
    evaluation: (userId: string | null | undefined) => ["evaluation", userId] as const,
};
...
export async function syncIdEmail(user: User) {
    const { error } = await supabase.from("user_data").upsert({ id: user.id, email: user.email }, { onConflict: "id" });
...
export async function fetchUserNrOrdem(userId: User["id"]): Promise<string | null> {
    const { data, error } = await supabase.from("user_data").select("nrOrdem").eq("id", userId).maybeSingle();
    if (error) throw error;
...
export async function fetchEvaluationForUser(userId: User["id"]): Promise<EvaluationResult> {
