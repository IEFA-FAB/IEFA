...
export const DefaultMessHallSelector = memo<DefaultMessHallSelectorProps>(
    ({ defaultMessHallCode, setDefaultMessHallCode, onApply, onCancel, isApplying }) => {
        ...
        const selectItems = useMemo(() => {
            if (!messHalls || messHalls.length === 0) {
                return <div className="px-2 py-4 text-sm text-muted-foreground">Nenhum rancho encontrado.</div>;
            }
                    ...
                    <CardDescription className="mt-3">
                        <div className="flex gap-2 rounded-md border p-2.5 bg-accent/10 text-accent-foreground border-accent/30">
                            ...
                            <span className="text-sm">
                                Defina um rancho padrão para os cards que ainda não possuem rancho definido no banco de
                                dados. Esta ação afetará apenas os cards sem rancho configurado.
                            </span>
                        </div>
                    </CardDescription>
                </CardHeader>
