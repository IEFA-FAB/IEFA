 ...
 *
 * As credenciais são lidas em UM lugar só (`src/test/supabase.ts`), e esse arquivo
 * já se governa por `SISUB_RUN_INTEGRATION`. Então o run unitário — que é o que roda
 * por padrão e no CI — não recebe nenhuma delas e passa a enxergar exatamente o env
 * do CI. `test:integration` liga a flag e recebe o env inteiro, como antes.
 *
 * As flags em si sempre passam: são elas que decidem se a integração roda, e lê-las
 * do `.env` é o que permite ligar a suíte sem exportar nada na linha de comando.
 */
...
function testEnv(): Record<string, string> {
	...
	const integration = (process.env.SISUB_RUN_INTEGRATION ?? all.SISUB_RUN_INTEGRATION) === "true"
