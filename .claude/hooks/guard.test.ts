...
const CASES: Array<[string, boolean | "ask", object]> = [
	...
	["repair applied", "ask", { tool_name: "Bash", tool_input: { command: "bunx supabase migration repair --status applied 1" } }],
	...
	["HUSKY=0", true, { tool_name: "Bash", tool_input: { command: "HUSKY=0 git commit -m x" } }],
	["hooksPath desviado", true, { tool_name: "Bash", tool_input: { command: "git -c core.hooksPath=/dev/null commit -m x" } }],
	["--no-veri abreviado", true, { tool_name: "Bash", tool_input: { command: "git commit --no-veri -m x" } }],
	["git -C push main", true, { tool_name: "Bash", tool_input: { command: "git -C /tmp/x push origin " + "main" } }],
	["repair --status=reverted", true, { tool_name: "Bash", tool_input: { command: "bunx supabase migration repair 1 --status=reverted" } }],
	["db:repair:reverted", true, { tool_name: "Bash", tool_input: { command: "bun --filter @iefa/database db:repair:reverted 1" } }],
	["db reset local", false, { tool_name: "Bash", tool_input: { command: "bunx supabase db reset" } }],
	["db reset linked", true, { tool_name: "Bash", tool_input: { command: "bunx supabase db reset --linked" } }],
	["db push pede", "ask", { tool_name: "Bash", tool_input: { command: "bunx supabase db push" } }],
	["db:push pede", "ask", { tool_name: "Bash", tool_input: { command: "bun --filter @iefa/database db:push" } }],
