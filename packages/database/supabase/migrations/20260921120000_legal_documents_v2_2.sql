...
-- mudanças são acréscimos e correções de fato, listadas abaixo. As versões anteriores
-- ficam como linhas próprias — `user_legal_acceptances.document_id` é FK `ON DELETE
-- RESTRICT`, e a prova de ciência delas continua intacta.
--
-- Motivo principal: o Contrate (contrate.iefa.com.br, `apps/contrate`), no ar desde
-- 2026-09-17 com a interface do Projeto α e as Facilidades do Pregoeiro, não aparecia em
-- documento nenhum — nem na lista de sistemas dos Termos e da Privacidade, nem nas linhas
-- do inventário de cookies que ele herdou do Portal. Um sistema com login e com cookie
-- operando sob documentos que não o nomeiam.
--
-- Termos e Privacidade (2.1.0 → 2.2.0): o Contrate entra na lista de sistemas cobertos
-- (seção 1 dos Termos, seção 2 da Privacidade). Nenhuma outra linha muda.
--
-- Política de Cookies (1.2.0 → 1.3.0), inventário conferido contra o código de todos os apps
-- e packages (`document.cookie`, `localStorage`, `sessionStorage`, `usePersistentState`,
-- cookies do `@supabase/ssr`, `@iefa/auth-kit`):
--   * Contrate: `theme` (cookie, 1 ano), `fab_remember_email` e as duas chaves do pregoeiro.
--     A barra lateral do Contrate NÃO é persistida (`apps/contrate/src/components/ui/
--     sidebar.tsx`), então ele não entra na linha de `sidebar_state`. A OM corrente vem da
--     URL, e a variante do acanto da página inicial é sorteada a cada carga — nenhuma das
--     duas é gravada;
--   * `pregoeiro_preferences_v1` e `pregoeiro_table_settings_v1` saíram do Portal com a
--     mudança do α para o Contrate (#351): a linha dizia "Portal" e o código só existe no
--     Contrate;
--   * SUCONT-4 grava `sidebar_state` (7 dias, `apps/sucont/src/components/ui/sidebar.tsx`,
--     desde o #254) e `theme` (1 ano, `apps/sucont/src/services/theme.tsx`, desde o #259) e
--     não constava de nenhuma das duas linhas;
--   * `auth_rate_limit` (armazenamento de sessão) é a trava de tentativas de login de
--     `@iefa/auth-kit`, usada pelas telas de login de SISUB, Portal, RUMAER, Formulários,
--     SUCONT-4 e Contrate. Nunca esteve no inventário porque o guard só varria `apps/`, e a
--     chave mora em `packages/`. O guard agora varre os dois;
--   * `rada_session_id:<conta>` (Portal, ChatRADA): guarda a conversa em andamento e leva no
--     NOME o identificador da conta. Escapava do guard por ser chave montada em template;
--   * SISUB: `sisub:cardapio-print-header:<escopo>` (armazenamento local) guarda o cabeçalho
--     do cardápio impresso, inclusive os NOMES de quem assina — dado pessoal, e por isso em
--     negrito junto com o e-mail e o rascunho do Portal. A linha de armazenamento de sessão
--     passa a cobrir as famílias reais de chave (`sisub:recipes:*`, `sisub:global-auxiliary:*`,
--     área de transferência do cardápio, rascunhos de cardápio novo), que também escapavam
--     por serem montadas em template;
--   * `sisub:stale-chunk-reloads` é `sessionStorage` (`apps/sisub/src/lib/recover-stale-
--     chunk.ts`), não armazenamento local como a 1.2.0 dizia;
--   * Documentação: o tema claro/escuro do Fumadocs é gravado em `localStorage` sob `theme`
--     (next-themes) — diferente dos demais apps, onde `theme` é cookie lido pelo servidor;
--   * o parágrafo dos itens em negrito passa a mencionar o cookie de sessão: o token dele
--     carrega o identificador e o e-mail da conta, e a frase antiga ("os itens em negrito
--     são os que guardam dado pessoal") deixava entender que ele não guardava.
--
