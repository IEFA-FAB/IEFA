-- ============================================================================
-- Conferência do recebimento: o SEM-LOTE do sistema não é divisão do operador,
-- e o total informado confere a quantidade da tela sob a trava
-- ============================================================================
--
-- Revisão de 20260921170000:
--
--  1. O "SEM-LOTE-<data>" que `sync_receipt_line` cria contava como lote do
--     operador. Com ele e o lote da nota, o resíduo não tinha mais destino: nota
--     L1 = 100, leitura de L1 × 50 e de uma caixa sem lote × 50, caixa desfeita,
--     total informado 100 → a soma dos lotes não fechava e a efetivação travava.
--  2. `record_receipt_event` ganha `p_expected_total`: o servidor conferia a
--     quantidade da tela e só depois gravava o total informado — a leitura que
--     entrasse no meio sumia. Agora a conferência é dentro da trava da linha.
-- ============================================================================
...
drop function inventory.record_receipt_event(uuid, uuid, text, text, numeric, uuid, text, text, text, date, numeric, uuid, text);
 ...
 * reconhecido como duplicado e, mesmo assim, a linha é recalculada.
 * `p_divergence_reason`, quando vem, é gravado na linha antes do recálculo —
 * na mesma transação e sob a mesma trava do evento. `p_expected_total`, quando
 * vem, recusa o evento se a linha não estiver mais com esse total.
 */
...
begin
  ...
  -- o total que a tela mostrava, conferido COM a linha travada: lido e
  -- comparado no servidor antes da chamada, uma leitura que entrasse no meio
  -- era apagada pelo total informado, sem aviso
  if p_expected_total is not null and exists (
    select 1 from inventory.goods_receipt_item where id = p_receipt_item_id and received_qty_base <> p_expected_total
  ) then
    raise exception 'A quantidade desta linha mudou enquanto você editava (outra leitura entrou) — recarregue e confira antes de salvar';
  end if;
  ...
  -- o motivo só acompanha o evento que ENTROU: o reenvio de uma recusa já
  -- superada não pode ressuscitar o "Recusado:"
  if v_id is not null and p_divergence_reason is not null then
    update inventory.goods_receipt_item
       set divergence_reason = nullif(btrim(p_divergence_reason), '')
     where id = p_receipt_item_id;
  end if;
