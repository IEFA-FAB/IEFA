-- Política de Cookies 1.5.0 (pt-BR + en-US). Vigência: 2026-09-26.
-- Termos de Uso 2.3.0 e Política de Privacidade 2.3.0 reinseridos INALTERADOS.
--
-- Supersede a Política de Cookies 1.4.0 de `20260926120000_legal_documents_cookies_v1_4.sql`,
-- cujo conteúdo é a base deste. Termos e Privacidade NÃO mudam: são reinseridos apenas para
-- que este arquivo continue sendo o conjunto vigente completo — é dele que os guards do
-- `@iefa/legal-kit` leem o texto publicado —, e o `ON CONFLICT DO NOTHING` do fim garante que
-- a reinserção não cria versão nova nem pede ciência de documento que ninguém alterou.
--
-- Motivo: o SISUB passa a guardar o rascunho das edições não salvas (insumo, itens de compra
-- e de produto, preparação) no armazenamento LOCAL do navegador, família `sisub:draft:*`,
-- para que F5, a recarga automática depois de uma publicação e fechar o navegador não apaguem
-- o que foi alterado. Antes o rascunho vivia só na memória da aba. Muda:
--   - uma linha nova no inventário da seção 3, nos dois idiomas;
--   - o aviso da seção 5 ("antes de limpar"), que passa a citar também esse rascunho;
--   - a view `legal_documents_current` passa a desempatar versões de mesma vigência.
--
-- Avaliação do dado (pedida pelo mantenedor): o rascunho guarda dado de CATÁLOGO que o próprio
-- usuário já vê — nome, pasta, unidade, nutrientes, especificação de compra com preço de
-- referência, ficha técnica. Não é dado pessoal nem classificado; por isso armazenamento
-- local, e não de sessão. Travas: validade de 7 dias sem uso, descarte quando outra conta
-- entra no navegador, e só uma assinatura curta da conta (FNV-1a de 32 bits, sem volta ao
-- identificador) — ver `apps/sisub/src/lib/drafts/draft-store.ts`.
--
...
-- Decisão do mantenedor (2026-09-26): publicar esta versão para o rascunho sobreviver ao F5.
--
-- APLICADA em 2026-09-26, antes do merge, em transação única com o
-- `INSERT INTO supabase_migrations.schema_migrations (version, name, statements)` no timestamp
-- exato deste arquivo.

-- ── Desempate da versão vigente ──────────────────────────────────────────────
-- `legal_documents_current` escolhia a vigente só por `effective_date DESC`. Duas versões
...
-- ============================================================
-- Termos de Uso — pt-BR 2.3.0 (inalterado)
-- ============================================================
(
  'terms_of_use',
  '2.3.0',
...
Estes Termos valem para: SISUB (Sistema de Subsistência), Portal IEFA, SUCONT-4, RUMAER, Formulários IEFA, Escolha de Vagas (CPAINT), Contrate, Projeto α, API IEFA e Documentação IEFA.
...
Pedidos de acesso, correção ou exclusão de dados devem ser enviados para **iefa@fab.mil.br**. Não existe exclusão automática pela interface — o procedimento está descrito na seção 10 da Política de Privacidade. A única exceção são as conversas com o assistente do Contrate, que o próprio usuário pode apagar pela tela, junto com os arquivos anexados a elas (seção 9 da Política de Privacidade).
...
-- ============================================================
-- Política de Privacidade — pt-BR 2.3.0 (inalterada)
-- ============================================================
(
  'privacy_policy',
  '2.3.0',
...
>
> **Não existe autoexclusão nos sistemas.** Pedidos de acesso, correção ou eliminação são atendidos manualmente pela Secretaria do IEFA — escreva para **iefa@fab.mil.br** e você tem resposta em até **7 dias corridos** (seção 10). A única exceção são as suas conversas com o assistente do Contrate, que você mesmo pode apagar (seção 9).
...
- **Contrate** — copiloto de aquisições do Projeto α: verificação de Estudo Técnico Preliminar e Termo de Referência, fila de conformidade e apoio à condução do pregão;
...
**Conteúdo produzido pelo usuário** — respostas a questionários (incluindo OM e seção do respondente), escolhas registradas na sessão de vagas, cardápios, receitas, planilhas e documentos enviados, e mensagens trocadas com os assistentes de inteligência artificial, inclusive os arquivos que você anexa às conversas com o assistente do Contrate.
...
**Não existe rotina de expurgo automático nos sistemas, com uma única exceção: as conversas avulsas com o assistente do Contrate, descritas abaixo.** Fora dela, nenhum dado é apagado por idade. Sendo direto sobre o que isso significa:
...
- **o histórico de conversas com os assistentes de IA** é armazenado junto ao registro do usuário e segue a mesma regra, salvo o do assistente do Contrate, a seguir.

**Conversas com o assistente do Contrate** — regra em vigor desde 22/09/2026:

- **conversa avulsa** (aberta em "Conversar", com os arquivos que você anexou): é apagada automaticamente, junto com os arquivos, depois de **180 dias sem uso**. A data aparece na lista de conversas. Se você **salvar** a conversa, ela fica guardada até você mesmo apagá-la;
- **conversa sobre um processo** (aberta na tela do processo): fica guardada enquanto o processo existir no Contrate;
- **você pode apagar** qualquer conversa sua, com os arquivos dela, pela própria tela e a qualquer momento. Apagar remove a conversa e os arquivos do armazenamento, e não há como desfazer;
- **a conversa é privada**: só quem a abriu a lê. Nem os demais servidores da OM nem o ACI do processo têm acesso a ela.

Declaramos isso expressamente porque a alternativa — anunciar um prazo de descarte que nenhuma rotina executa — seria informação falsa. Toda rotina de expurgo é publicada aqui com a respectiva data de início, como a das conversas do Contrate.
...
**Não existe botão de autoexclusão nos sistemas do IEFA.** Nenhum usuário consegue apagar a própria conta ou os próprios dados pela interface. Toda solicitação — inclusive a de exclusão — é recebida por e-mail e **processada manualmente** pela Secretaria do IEFA. A única exceção é a conversa com o assistente do Contrate, que você mesmo pode apagar pela tela (seção 9); para a conta e os demais dados, o canal é o e-mail.
...
-- ============================================================
-- Política de Cookies — pt-BR 1.5.0
-- ============================================================
(
  'cookie_policy',
  '1.5.0',
...
| `fab_remember_email` | armazenamento local | SISUB, Portal, RUMAER, Formulários, Contrate | até ser limpo pelo usuário | **guarda o seu e-mail institucional** para preencher a tela de login; só é gravado se você marcar "lembrar" |
...
| `auth_rate_limit` | armazenamento de sessão | SISUB, Portal, RUMAER, Formulários, SUCONT-4, Contrate | fim da aba | contar tentativas de login malsucedidas e o fim da espera imposta depois delas; não guarda e-mail nem senha |
| `sidebar_state` | cookie | SISUB, Formulários, SUCONT-4 | 7 dias | lembrar se a barra lateral está recolhida |
| `theme` | cookie | SISUB, Portal, RUMAER, Formulários, SUCONT-4, Contrate | 1 ano | tema claro ou escuro; é lido pelo servidor para já enviar a página no tema certo |
| `theme` na Documentação | armazenamento local | Documentação IEFA | até ser limpo pelo usuário | tema claro ou escuro da documentação; fica só no navegador, sem ser lido pelo servidor |
...
| `sisub:cardapio-print-header:<escopo>` | armazenamento local | SISUB | até ser limpo pelo usuário | **guarda o cabeçalho do cardápio impresso, inclusive o nome de quem assina** (organização, seção, título e as quatro assinaturas), para não redigitá-lo a cada semana; um por cozinha e um para o plano modelo |
| `sisub:stale-chunk-reloads` | armazenamento de sessão | SISUB | fim da aba | evitar laço de recarregamento após uma publicação |
| `sisub:global-ingredients:*`, `sisub:global-auxiliary:*`, `sisub:global-preparations:*`, `sisub:recipes:*`, `sisub:ingredients:findReplace:*` | armazenamento de sessão | SISUB | fim da aba | posição de rolagem, busca, filtros, ordenação e pastas abertas das listas de insumos, preparações e receitas, e as opções da busca e substituição |
| `sisub:menu:*`, `sisub:weekly-plan:clipboard`, `sisub:day-menu:clipboard:<cozinha>`, `sisub:recipe-form:quantity-basis` | armazenamento de sessão | SISUB | fim da aba | itens copiados entre dias do cardápio, tipo de demanda padrão e a forma de informar a quantidade na ficha técnica |
| `weekly-menus-new-draft-<cozinha>`, `events-new-draft-<cozinha>`, `exceptions-new-draft-<cozinha>` | armazenamento de sessão | SISUB | fim da aba, ou até o cardápio ser criado | rascunho do nome e da descrição do cardápio que está sendo criado e, nos apoios (lanches de bordo e de apoio, coffee breaks), das ocorrências mensais; no catálogo global da SDAB, a chave termina em global em vez do número da cozinha |
| `sisub:draft:*` | armazenamento local | SISUB | até você salvar ou descartar a edição, até 7 dias sem uso, ou até outra conta entrar no mesmo navegador | rascunho das edições ainda não salvas de insumos, itens de compra, itens de produto e preparações, para que recarregar a página ou fechar o navegador não apague o que foi alterado; guarda também uma assinatura curta da conta dona dos rascunhos (não o identificador dela), só para descartá-los se outra conta entrar. O conteúdo é dado de catálogo, não dado pessoal, e não é enviado ao servidor antes de você salvar |
| `iefa_app_favorites_v1` | armazenamento local | Portal | até ser limpo pelo usuário | aplicações marcadas como favoritas |
| `rada_session_id:<conta>` | armazenamento local | Portal | até ser limpo pelo usuário, ou até você iniciar nova conversa | retomar a conversa em andamento no ChatRADA; **o nome da chave inclui o identificador da sua conta**, para que outra pessoa no mesmo computador não retome a sua conversa |
| `pregoeiro_preferences_v1` | armazenamento local | Contrate | até ser limpo pelo usuário | preferências do painel do pregoeiro, quando usado sem login (com login, ficam na sua conta) |
| `pregoeiro_table_settings_v1` | armazenamento local | Contrate | até ser limpo pelo usuário | colunas e ordenação das tabelas do pregoeiro, quando usado sem login (com login, ficam na sua conta) |
...
O cookie de sessão carrega o token de autenticação, e dentro dele o identificador e o e-mail da sua conta; o cookie vai apenas aos servidores dos sistemas do IEFA, a cada requisição, e o token é apresentado ao Supabase, operador de autenticação descrito na seção 6 da Política de Privacidade. Além dele, os itens em negrito são os que guardam **dado pessoal** no seu navegador: o e-mail institucional que você digitou na tela de login, o rascunho da comunicação oficial em redação, que carrega nome, posto e cargo de quem assina e de quem recebe, o cabeçalho do cardápio impresso, com o nome de quem o assina, e o identificador da sua conta no nome da chave do ChatRADA. Todos ficam apenas no seu dispositivo e não são enviados a terceiros. Limpar o armazenamento do navegador remove todos eles; no caso do e-mail, desmarcar "lembrar" já basta.
...
Atenção antes de limpar: o rascunho de comunicação oficial e o rascunho de edição do SISUB que ainda não foram salvos vivem no armazenamento local. Limpá-lo descarta o que não tiver sido gravado no servidor, e não há como recuperá-lo.
...
-- ============================================================
-- Terms of Use — en-US 2.3.0 (unchanged)
-- ============================================================
(
  'terms_of_use',
  '2.3.0',
...
These Terms apply to: SISUB, IEFA Portal, SUCONT-4, RUMAER, IEFA Forms, Assignment Selection (CPAINT), Contrate, Project α, IEFA API and IEFA Documentation.
...
Requests for access, correction or deletion of data must be sent to **iefa@fab.mil.br**. There is no automated deletion through the interface — the procedure is described in section 10 of the Privacy Policy. The only exception is conversations with the Contrate assistant, which users can delete themselves on screen, together with the files attached to them (section 9 of the Privacy Policy).
...
-- ============================================================
-- Privacy Policy — en-US 2.3.0 (unchanged)
-- ============================================================
(
  'privacy_policy',
  '2.3.0',
...
>
> **There is no self-service deletion.** Requests for access, correction or deletion are handled manually by the IEFA Secretariat — write to **iefa@fab.mil.br** and you get an answer within **7 calendar days** (section 10). The only exception is your conversations with the Contrate assistant, which you can delete yourself (section 9).
...
- **Contrate** — Project α procurement copilot: review of Preliminary Technical Studies and Terms of Reference, compliance queue and support for running the electronic auction;
...
**There is no automated data-purge routine in the systems, with a single exception: ad-hoc conversations with the Contrate assistant, described below.** Apart from that, no data is deleted based on age. To be direct about what that means:
...
- **AI assistant conversation history** is stored alongside the user record and follows the same rule, except for the Contrate assistant, below.

**Conversations with the Contrate assistant** — rule in force since 22 September 2026:

- **ad-hoc conversation** (opened under "Conversar", with files you attached): deleted automatically, together with its files, after **180 days without use**. The date is shown in the conversation list. If you **save** the conversation, it is kept until you delete it yourself;
- **conversation about a process** (opened on the process screen): kept for as long as the process exists in Contrate;
- **you can delete** any of your conversations, with their files, on screen at any time. Deleting removes the conversation and the files from storage, and cannot be undone;
- **the conversation is private**: only the person who opened it can read it. Neither the other staff of the OM nor the process's internal control analyst have access to it.

We state this explicitly because the alternative — announcing a disposal deadline that no routine enforces — would be false information. Every purge routine is published here with its start date, like the one for Contrate conversations.
...
**There is no self-service deletion button in IEFA systems.** No user can delete their own account or data through the interface. Every request — deletion included — is received by e-mail and **processed manually** by the IEFA Secretariat. The only exception is conversations with the Contrate assistant, which you can delete yourself on screen (section 9); for your account and any other data, the channel is e-mail.
...
| `fab_remember_email` | local storage | SISUB, Portal, RUMAER, Forms, Contrate | until cleared by the user | **stores your institutional e-mail** to prefill the sign-in screen; only written if you tick "remember" |
...
| `auth_rate_limit` | session storage | SISUB, Portal, RUMAER, Forms, SUCONT-4, Contrate | until the tab closes | counting failed sign-in attempts and when the waiting period imposed after them ends; stores neither e-mail nor password |
| `sidebar_state` | cookie | SISUB, Forms, SUCONT-4 | 7 days | remembering whether the sidebar is collapsed |
| `theme` | cookie | SISUB, Portal, RUMAER, Forms, SUCONT-4, Contrate | 1 year | light or dark theme; read by the server so the page is sent already in the right theme |
| `theme` in the Documentation | local storage | IEFA Documentation | until cleared by the user | light or dark theme of the documentation; stays in the browser and is not read by the server |
...
| `sisub:cardapio-print-header:<scope>` | local storage | SISUB | until cleared by the user | **stores the header of the printed menu, including the names of the signers** (organisation, section, title and the four signatures), so it does not have to be retyped every week; one per kitchen and one for the model plan |
| `sisub:stale-chunk-reloads` | session storage | SISUB | until the tab closes | preventing a reload loop after a deployment |
| `sisub:global-ingredients:*`, `sisub:global-auxiliary:*`, `sisub:global-preparations:*`, `sisub:recipes:*`, `sisub:ingredients:findReplace:*` | session storage | SISUB | until the tab closes | scroll position, search, filters, sort order and expanded folders of the ingredient, preparation and recipe lists, and the find-and-replace options |
| `sisub:menu:*`, `sisub:weekly-plan:clipboard`, `sisub:day-menu:clipboard:<kitchen>`, `sisub:recipe-form:quantity-basis` | session storage | SISUB | until the tab closes | items copied between menu days, the default demand type and how quantities are entered on the recipe sheet |
| `weekly-menus-new-draft-<kitchen>`, `events-new-draft-<kitchen>`, `exceptions-new-draft-<kitchen>` | session storage | SISUB | until the tab closes, or until the menu is created | draft of the name and description of the menu being created and, for support meals ("Apoios": in-flight and field snacks, coffee breaks), of the monthly occurrences; in the SDAB global catalog, the key ends in global instead of the kitchen number |
| `sisub:draft:*` | local storage | SISUB | until you save or discard the edit, after 7 days without use, or until another account signs in on the same browser | draft of unsaved edits to ingredients, purchase items, product items and preparations, so that reloading the page or closing the browser does not erase the changes; it also keeps a short signature of the account that owns the drafts (not its identifier), only to discard them if another account signs in. The content is catalogue data, not personal data, and it is not sent to the server until you save |
| `iefa_app_favorites_v1` | local storage | Portal | until cleared by the user | applications marked as favourites |
| `rada_session_id:<account>` | local storage | Portal | until cleared by the user, or until you start a new conversation | resuming the ongoing ChatRADA conversation; **the key name includes your account identifier**, so that another person on the same computer does not resume your conversation |
| `pregoeiro_preferences_v1` | local storage | Contrate | until cleared by the user | preferences for the procurement officer dashboard, when used without signing in (when signed in, they are kept in your account) |
| `pregoeiro_table_settings_v1` | local storage | Contrate | until cleared by the user | column and sort settings for the procurement officer tables, when used without signing in (when signed in, they are kept in your account) |
...
The session cookie carries the authentication token, and inside it your account identifier and e-mail; the cookie is sent only to the servers of IEFA systems, with every request, and the token is presented to Supabase, the authentication processor described in section 6 of the Privacy Policy. Beyond it, the entries in bold are the ones that keep **personal data** in your browser: the institutional e-mail you typed on the sign-in screen, the draft of the official communication being written, which carries the name, rank and position of the signer and of the addressees, the header of the printed menu, with the names of its signers, and your account identifier in the name of the ChatRADA key. All of them stay on your device and are not sent to third parties. Clearing browser storage removes them all; for the e-mail, unticking "remember" is enough.
...
One warning before clearing: the draft of an official communication and the SISUB edit draft that have not been saved live in local storage. Clearing it discards whatever was not written to the server, and it cannot be recovered.
