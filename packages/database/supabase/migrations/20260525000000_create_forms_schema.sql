...
begin
  if not exists (
    ...
    where n.nspname = 'forms' and t.typname = 'questionnaire_status'
  ) then
  ...
  if not exists (
    ...
    from pg_type t
    join pg_namespace n on n.oid = t.typnamespace
    where n.nspname = 'forms' and t.typname = 'questionnaire_response_status'
  ) then
  ...
  if not exists (
    ...
    from pg_type t
    join pg_namespace n on n.oid = t.typnamespace
    where n.nspname = 'forms' and t.typname = 'question_type'
  ) then
...
-- questionnaire
create table if not exists forms.questionnaire (
...
-- section
create table if not exists forms.section (
  id uuid primary key default gen_random_uuid(),
...
-- question
create table if not exists forms.question (
  id uuid primary key default gen_random_uuid(),
...
create table if not exists forms.questionnaire_response (
  id uuid primary key default gen_random_uuid(),
  questionnaire_id uuid not null references forms.questionnaire(id) on delete cascade,
...
create table if not exists forms.response (
  id uuid primary key default gen_random_uuid(),
...
-- indexes
create index if not exists idx_section_order on forms.section(questionnaire_id, sort_order);
create index if not exists idx_question_order on forms.question(section_id, sort_order);
create index if not exists idx_qr_respondent on forms.questionnaire_response(questionnaire_id, respondent_id, status);
create unique index if not exists idx_qr_single_draft on forms.questionnaire_response(questionnaire_id, respondent_id) where status = 'draft';
create index if not exists idx_response_session on forms.response(questionnaire_response_id);
...
drop trigger if exists trg_questionnaire_updated_at on forms.questionnaire;
...
drop trigger if exists trg_response_updated_at on forms.response;
