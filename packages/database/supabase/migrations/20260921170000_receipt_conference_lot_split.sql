-- ============================================================================
-- Conferência do recebimento: divisão de lotes do operador, arredondamento dos
-- lotes lidos e recusa numa transação só
-- ============================================================================
--
-- Revisão de 20260920250000:
--
--  1. O resíduo da linha ia SEMPRE para o lote com o código da nota, mesmo com
--     a entrega dividida pelo operador em outros lotes — L1 60 + L2 40 de uma
--     nota com L1 = 100 voltava a L1 100 a cada leitura, e a efetivação travava
--     em "Soma dos lotes difere". 20260920240000 respeitava a divisão; esta
--     volta a respeitar, sem perder o que a 250000 ganhou (o lote da nota nunca
--     se apaga, e recebe o resíduo quando é o único destino).
--  2. O total ajustado ao faturado (9,9999 → 10) deixava 0,0001 fora dos lotes
--     lidos, e esse resto virava um lote de estoque sem validade.
--  3. `record_receipt_event` ganha `p_divergence_reason`: a recusa gravava o
--     motivo e o evento em duas transações — uma leitura no meio apagava o
--     motivo, e a falha do segundo passo deixava a linha "Recusado:" com a
--     quantidade inteira. O total informado com motivo usa o mesmo caminho.
-- ============================================================================
...
begin
  ...
  if v_manual_count > 1 then
    ...
    if v_manual_count = 0 and v_residual > 0 then
      insert into inventory.goods_receipt_item_lot (receipt_item_id, lot_code, expiry_date, quantity_base, unit_cost)
...
drop function inventory.record_receipt_event(uuid, uuid, text, text, numeric, uuid, text, text, text, date, numeric, uuid);
 ...
 * na mesma transação e sob a mesma trava do evento.
 */
