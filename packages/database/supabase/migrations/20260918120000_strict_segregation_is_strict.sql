-- ============================================================================
-- Segregação estrita passa a ser estrita
-- ============================================================================
-- O ramo `segregation = 'strict'` em `post_stock_adjustment` tinha EXATAMENTE a
-- mesma guarda do `if` acima dele:
--
--     if v_doc.created_by is not distinct from p_actor and p_approval_exception_reason is null then ...
--     if v_settings.segregation = 'strict' and v_doc.created_by is not distinct from p_actor
--        and p_approval_exception_reason is null then ...
--
-- A segunda condição nunca era alcançada: quando ela seria verdadeira, a
-- primeira já havia levantado exceção. Resultado: `strict` era byte a byte
-- igual a `dual`, e o nível 3 sozinho continuava se autoaprovando pelo caminho
-- da exceção — exatamente o que a coluna promete impedir.
--
-- A regra correta separa os dois regimes:
--   • `strict`  — aprovador NUNCA é o autor. Sem segunda pessoa, o ajuste
--     espera. É o regime de quem tem gente para isso.
--   • `dual`    — sem segunda pessoa elegível, a operação segue com a exceção
--     REGISTRADA (o servidor é quem atesta que não há outro nível 3), porque
--     travar a cozinha no sábado é o que faz o controle virar contorno.
-- ============================================================================
