-- ============================================================================
-- Aprovar a contagem espera a requisição da PRODUÇÃO, não qualquer uma
-- ============================================================================
-- A pré-condição "produção concluída sem a requisição do dia fechada" aceitava
-- QUALQUER requisição fechada naquela data. Uma saída avulsa fechada (o apoio
-- da manhã) satisfazia a regra com a requisição da produção ainda aberta — e a
-- contagem aprovada acusava como falta o que a produção já tinha tirado. Com
-- várias avulsas por dia (20260918220000), o caso deixou de ser raro.
-- ============================================================================
     ...
     and not exists (
       select 1 from inventory.stock_issue_request r
        where r.kitchen_id = t.kitchen_id
          ...
          -- só a requisição da PRODUÇÃO fecha a baixa da produção: uma avulsa
          -- fechada no mesmo dia não diz nada sobre o que o cardápio consumiu
