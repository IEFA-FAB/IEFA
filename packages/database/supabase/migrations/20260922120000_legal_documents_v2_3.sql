-- Termos de Uso 2.3.0 e Política de Privacidade 2.3.0 (pt-BR + en-US). Vigência: 2026-09-22.
-- Política de Cookies 1.3.0 reinserida INALTERADA.
--
-- Supersede Termos e Privacidade 2.2.0 de `20260921120000_legal_documents_v2_2.sql`, cujo
-- conteúdo é a base deste. A Política de Cookies NÃO muda: o chat do Contrate não grava
-- cookie nem armazenamento local. Ela é reinserida aqui apenas para que este arquivo continue
-- sendo o conjunto vigente completo — é dele que os guards do `@iefa/legal-kit` leem o texto
-- publicado —, e o `ON CONFLICT DO NOTHING` do fim garante que a reinserção não cria versão
-- nova nem pede ciência de documento que ninguém alterou.
--
-- Motivo: o chat sobre documento do Contrate (change `alpha-chat-processo`, tabelas
-- `alpha.chat_*` em `20260922034921_alpha_document_chat.sql`) cria as duas primeiras
-- exceções às declarações da 2.2.0, e a política não pode afirmar o contrário do que o
-- sistema faz:
--   * EXPURGO: a conversa avulsa não salva é apagada, com os anexos, após 180 dias sem uso
--     (`apps/alpha/src/jobs/purge-chats.ts`). A 2.2.0 dizia que não havia rotina nenhuma e
--     prometia publicar a primeira com data de início — é o que a seção 9 faz agora;
--   * APAGAR PELA TELA: o usuário apaga a própria conversa e os arquivos dela. Decisão do
--     mantenedor (2026-09-22): apagar conversa do assistente é descartar rascunho de
--     trabalho, não exercício de direito do titular. A conta e todo o resto seguem sem
--     autoexclusão, só pelo e-mail — as frases que o `contact.test.ts` cobra continuam;
--   * os anexos entram no "conteúdo produzido pelo usuário" (seção 4).
-- Os Termos ganham a mesma exceção em uma frase, na seção que remete à Política de Privacidade. Nenhuma outra linha muda.
--
...
-- ============================================================
-- Política de Privacidade — pt-BR 2.2.0
-- ============================================================
(
  'privacy_policy',
  '2.3.0',
...
-- ============================================================
-- Política de Cookies — pt-BR 1.3.0
-- ============================================================
(
  'cookie_policy',
  '1.3.0',
...
| `weekly-menus-new-draft-<cozinha>`, `events-new-draft-<cozinha>`, `exceptions-new-draft-<cozinha>` | armazenamento de sessão | SISUB | fim da aba, ou até o cardápio ser criado | rascunho do nome e da descrição do cardápio que está sendo criado e, em eventos e exceções, das ocorrências |
...
| `weekly-menus-new-draft-<kitchen>`, `events-new-draft-<kitchen>`, `exceptions-new-draft-<kitchen>` | session storage | SISUB | until the tab closes, or until the menu is created | draft of the name and description of the menu being created and, for events and exceptions, of the occurrences |
