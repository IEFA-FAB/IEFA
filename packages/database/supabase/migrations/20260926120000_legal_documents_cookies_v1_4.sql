-- Política de Cookies 1.4.0 (pt-BR + en-US). Vigência: 2026-09-26.
-- Termos de Uso 2.3.0 e Política de Privacidade 2.3.0 reinseridos INALTERADOS.
--
-- Supersede a Política de Cookies 1.3.0 de `20260922132000_legal_documents_v2_3.sql`, cujo
-- conteúdo é a base deste. Termos e Privacidade NÃO mudam: são reinseridos aqui apenas para
-- que este arquivo continue sendo o conjunto vigente completo — é dele que os guards do
-- `@iefa/legal-kit` leem o texto publicado —, e o `ON CONFLICT DO NOTHING` do fim garante
-- que a reinserção não cria versão nova nem pede ciência de documento que ninguém alterou.
--
-- Motivo: o SISUB renomeou na tela o artefato "Exceção" para "Apoio" (#434). Engloba os
-- lanches de bordo e de apoio do Módulo 7 e os demais apoios previsíveis (coffee break,
-- café de reunião). A linha do inventário da seção 3 que descreve o rascunho de criação de
-- cardápio dizia "em eventos e exceções, das ocorrências" — nomeava uma tela que o usuário
-- não encontra mais, e as ocorrências mensais só existem no apoio, não no evento. Muda só
-- essa linha, nos dois idiomas. A chave `exceptions-new-draft-<cozinha>` continua igual: o
-- código segue chamando o artefato de `exception`, e o inventário lista a chave literal que
-- o navegador grava.
--
-- Efeito esperado: a Política de Cookies é linha nova, então o aviso de ciência reaparece
-- para todo usuário de todo app com sessão (sem bloquear a navegação — ver LGPD.md).
-- Decisão do mantenedor (2026-09-26): publicar agora, em vez de esperar a próxima revisão.
--
...
-- ============================================================
-- Política de Cookies — pt-BR 1.4.0
-- ============================================================
(
  'cookie_policy',
  '1.4.0',
