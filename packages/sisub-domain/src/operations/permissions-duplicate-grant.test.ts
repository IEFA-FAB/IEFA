 ...
 * outra concedendo. `user_permissions_grant_uniq` (migração 20260917183328) passou a
 * recusar a segunda linha em QUALQUER módulo e QUALQUER escopo.
 *
 * O que este teste fixa é o que a recusa vira para quem está na tela: uma mensagem que
 * diz o que fazer, e não o 23505 cru — e, principalmente, que ela NÃO engole as outras
 * falhas de driver.
 */
...
import { createUserPermission, isDuplicateGrantViolation } from "./permissions.ts"
...
describe("isDuplicateGrantViolation", () => {
	test("reconhece o 23505 de user_permissions_grant_uniq escondido em .cause", () => {
		expect(isDuplicateGrantViolation(driverError({ code: "23505", constraint_name: "user_permissions_grant_uniq" }))).toBe(true)
	})
	...
	test("não confunde com outro SQLSTATE na mesma constraint", () => {
		expect(isDuplicateGrantViolation(driverError({ code: "23503", constraint_name: "user_permissions_grant_uniq" }))).toBe(false)
	})
...
describe("createUserPermission", () => {
	test("traduz a duplicata em PERMISSION_ALREADY_EXISTS apontando a concessão existente", async () => {
		const db = failingDb(driverError({ code: "23505", constraint_name: "user_permissions_grant_uniq" }))
