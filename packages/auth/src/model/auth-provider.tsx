...
export interface AuthProviderProps<
    DB = any,
    SN extends (string & Exclude<keyof DB, "__InternalSupabase">) | { PostgrestVersion: string } = "public",
> {
...
>({ supabase, children }: AuthProviderProps<DB, SN>) {
    ...
    const signUp = useCallback(
        async (email: string, password: string, redirectTo?: string) => {
            try {
                ...
                if (e?.message) throw e;
                throw new Error("Não foi possível cadastrar. Tente novamente mais tarde.");
            }
        },
        [supabase],
    );
    ...
    const value = useMemo<AuthContextType>(
        ...
        [user, session, isLoading, signIn, signUp, signOut, resetPassword, refreshSession],
    );
