...
export function getRedirectCandidates(locationSearch: string, locationState: any | undefined, redirectKey: string) {
...
export function getRedirectTo(locationSearch: string, locationState?: any): string | null {
    const params = new URLSearchParams(locationSearch || "");
    const qsTarget = params.get("redirectTo");
...
export function preserveRedirectFromQuery(locationSearch: string, redirectKey: string) {
    const params = new URLSearchParams(locationSearch || "");
...
export function resolveTarget(
    ...
    const { qsTarget, stateTarget, stored } = getRedirectCandidates(locationSearch, locationState, redirectKey);
    const target = safeRedirect(qsTarget ?? stored ?? stateTarget, defaultRedirect);
