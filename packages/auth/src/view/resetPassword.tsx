...
import { type Location as RouterLocation, useLocation, useNavigate } from "react-router";
import { useAuth } from "../model/auth-provider";
import { getRedirectTo, safeRedirect } from "../model/redirect";
...
export function ResetPassword({ supabase }: ResetPasswordProps) {
    ...
    }, [location.hash, location.search, location]);
    ...
    useEffect(() => {
        ...
        async function ensureSession() {
            ...
            try {
                ...
                if (!isMounted) return;
                setVerifyError("Link de recuperação inválido ou expirado. Solicite novamente.");
                setVerified(false);
                setIsVerifying(false);
            } catch (e: any) {
                if (!isMounted) return;
                setVerifyError(e?.message || "Não foi possível validar o link de recuperação. Solicite novamente.");
                setVerified(false);
                setIsVerifying(false);
            }
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!verified) {
            setApiError("Sessão de recuperação não encontrada. Reabra o link do e-mail.");
            return;
        }
        ...
        try {
            ...
            const target = safeRedirect(getRedirectTo(location.search, location.state), "/rancho");
            setTimeout(() => navigate(target, { replace: true }), 1200);
        } catch (err: any) {
            setApiError(err?.message || "Falha ao atualizar a senha. Tente novamente.");
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <Card className="w-full max-w-md mx-auto">
            <CardHeader className="space-y-1">
                ...
                <CardDescription className="text-center">Crie sua nova senha para acessar o SISUB</CardDescription>
            </CardHeader>
