        ...
        return (
            <div className="h-full w-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
                <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
                    {cfg.brand.logoUrl ? (
                        <img src={cfg.brand.logoUrl} alt={cfg.brand.title} className="mx-auto h-16" />
                    ) : (
                        <h1 className="text-7xl md:text-8xl text-blue-600 font-black">{cfg.brand.title}</h1>
                    )}
                    {cfg.brand.subtitle && <p className="text-gray-600">{cfg.brand.subtitle}</p>}
                </div>
                <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
                    <div className="w-full flex items-center justify-center py-12 bg-white rounded-md shadow-sm">
                        {LoaderIcon ? <LoaderIcon className="h-6 w-6 animate-spin text-gray-500" /> : null}
                        <span className="ml-2 text-sm text-gray-600">{cfg.ui.checkingAuth}</span>
                    </div>
                </div>
            </div>
        );
            ...
            <div className="sm:mx-auto sm:w-full sm:max-w-md">
                <div className="text-center">
                    {cfg.brand.logoUrl ? (
                        ...
                        <h1 className="text-7xl md:text-8xl font-black text-blue-600">{cfg.brand.title}</h1>
                    )}
                    {cfg.brand.subtitle && <p className="text-gray-600">{cfg.brand.subtitle}</p>}
                </div>
            </div>
