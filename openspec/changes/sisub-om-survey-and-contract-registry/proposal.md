...
1. **O formato já foi resolvido uma vez — para outro grão.** A matriz de efetivo (PR #243/#245) virou `kitchen.workforce_survey`/`workforce_submission`/`workforce_headcount`, com operations, telas e reset de treino, e está em produção (1 competência, 32 submissões). É a prova de que levantamento por planilha vira série histórica no sisub. Mas aquele motor responde **por rancho**, com **taxonomia global** de quadros e **contagem inteira** — o DIVISA responde **por OM**, com **categoria por campanha** e **valor em reais** mais texto. Ver a decisão 0 do `design.md` sobre por que o DIVISA ganha tabelas irmãs em vez de generalizar aquelas.
...
- **Pré-condição dura para (c): `core.units.uasg`** — preenchida em poucas OMs. A curadoria **não** entra neste change: já tem dono em `archive/2026-09-08-sisub-pncp-integration` (tarefa 0.1), pela tela `/unit/$unitId/settings`, com `fetchPcaUasgsFn` e `/analytics/procurement-plan` (#266) de apoio. BABV e CINDACTA também já foram decididos como dado no #243 (`kitchen.rancho.elo_code`; seed da matriz) e não viram migration aqui. O worker de (c) roda sobre as UASGs que existirem e reporta as OMs sem UASG, em vez de esperar a lista completa.
...
- **packages/database**: uma migration no schema `kitchen` (6 tabelas novas), RLS ligada sem policy como as irmãs e grants para `service_role`. Sem escrita em `core.units` — a curadoria de UASG segue pela tela (ver acima). Regenerar `generated.ts` e o schema Drizzle. **Nenhuma view `SECURITY DEFINER`** — o consolidado é calculado no domínio, não em view (ver `20260825155457_security_definer_views_and_anon_rpc_lockdown.sql`).
- **packages/sisub-domain**: `operations/survey.ts` e `operations/subsistence-contract.ts`; `utils/survey-crosswalk.ts` (sugestão) e `utils/survey-consolidation.ts` (totais), puros e testáveis sem banco. `operations/training.ts` ganha os passos de reset, com as tabelas filhas **antes** de `survey_response` e `subsistence_contract`, senão o reset de treino cai por FK. As tabelas entram no contrato de reset **antes** da migration ser aplicada (declara → aplica → mergeia), senão o contrato default-deny derruba a `main`.
...
- **apps/api**: worker `contratacoes-sync` contra `dadosabertos.compras.gov.br`, no padrão de `workers/compras-sync`, gravando em `compras_gov_integration.integration_sync_log`/`integration_sync_step` com `source` próprio — a trava `uq_integration_sync_log_one_running_per_source` é por `source`, e reusar o de outro worker faria os dois disputarem a mesma vaga.
