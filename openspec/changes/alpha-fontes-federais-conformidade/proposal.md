...
- **Consolidação do alpha no projeto Supabase do sisub, sob o schema `alpha`**: as migrations passam a viver em `packages/database/supabase/migrations/`, no mesmo histórico dos demais schemas, e o deploy herda os secrets do sisub. O schema nasce **vazio**: o projeto Supabase antigo foi apagado em 2026-07-31, com o corpus do RADA indexado dentro e sem backup. Reingerir a partir de Markdown passa a ser o único caminho, e o ChatRADA fica sem base até isso acontecer.
...
- **Banco (`packages/database`, projeto Supabase do sisub, schema `alpha`)**: extensões `vector` e `ltree`; tabelas do corpus (`document`, `document_chunk`, `query_log`) já com versionamento desde a origem; tabelas novas `normative_source`, `structure_node`, `explanatory_note`, `placeholder`, `checklist_rule`, `submission`, `extraction`, `compliance_run`, `compliance_finding`; RPCs `alpha.match_chunks_cosine` e `alpha.match_chunks_fts`; schema `alpha` exposto no PostgREST (`pgrst.db_schemas` + reload) e incluído nos scripts `db:types` / `db:pull` / `db:diff`.
